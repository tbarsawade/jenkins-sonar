<%@ page title="" language="VB" masterpagefile="~/usrFullScreenBPM.master" autoeventwireup="false" inherits="Documents, App_Web_tkfk5fna" viewStateEncryptionMode="Always" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server" EnableViewState="false">
  <link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-beta.1/dist/css/select2.min.css" rel="stylesheet" />
    ...
    <script src="js_child/jquery.min.cache"></script>
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-beta.1/dist/js/select2.min.js"></script>
    <script type="text/javascript">
        ...
        $(function () {
            $("input.txtNumber,input.txtHeaderNumber,input.calnum").css("text-align", "right");
            $("input.txtNumber,input.txtHeaderNumber").change(function () {
                var decimalValue = $(this).attr("allowdecimalvalue");
                if (decimalValue > 0) {
                    var number = this.value.split('.');
                    if (number.length > 1) {
                        if (number[1].length > parseInt(decimalValue)) {
                            alert("After decimal point upto " + decimalValue + " digits are allowed.");
                            var num = this.value;
                            $(this).val(toFixedTrunc(num, decimalValue));
                            //$(this).val(parseFloat(num).toFixed(decimalValue));
                            //$(this).val($(this).val().slice(0, $(this).val().length - (decimalValue-1)));
                            return false;
                        }
                    }
                }
            ...
            $("input.txtNumber,input.txtHeaderNumber").keydown(function (e) {
                ...
                if (($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190,109,189]) !== -1 ||
                    ...
                    if (number.length > 1 && (key == 190 || key == 110)) {
                        $("#ContentPlaceHolder1_lblTab").text("Number field can accept only one decimal point.");
                        return false;
                    }
                    return;
                }
               ...
               var charValue = String.fromCharCode(e.keyCode)
                ...
                if (!valid) {
                    ...
                    if ($(this).hasClass('txtHeaderNumber')) {
                        ...
                        $("#ContentPlaceHolder1_lblTab").text(columnName + " accept only numeric digit.");
                    }
                    else {
                        var col = $(this).parent('td').index();
                        var row = $(this).parent('td').closest('tr').index();
                        columnName = $(this).closest('table').find("th:eq(" + col + ")").text();
                        $("#ContentPlaceHolder1_lblTab").text(columnName + " at line no " + row + " accept only numeric digit.");
                    }
    ...
    <script type="text/javascript">
        ...
        function pageLoad(sender, args) {
            ...
            if (args.get_isPartialLoad()) {
                ...
                // Commented By Manvendra 13-06-2019
                ...
                //uncommented by manvendra 21/aug/2019
                var objgrid1 = $("table.mGrid");
                objgrid1.each(function () {
                    var parent = $(this).parent().parent().parent();
                    parent.show();
        ...
        $(document).ready(function () {
            UtilJs.PreservFile();
            var j = 0;
            $("#ContentPlaceHolder1_pnlFields").find("input,textarea,select,img").each(function (i) {
                if (!$(this).hasClass('invisible')){
                    if (!$(this).prop('disabled')) {
                        $(this).attr('tabindex', i + 1);
                        $(this).addClass('on-focus-shadow');
                        if (j == 0 && !$(this).attr('readonly')) {
                            $(this).focus();
                            j = j + 1;
                        }
                    }
                                            ...
                                            <td>
                                                ...
                                                <div style="width: 100%; text-align: right">
                                                    ...
                                                    <asp:Button ID="Button1" runat="server" Text="Calculate" Visible="false" CssClass="btnNew" Font-Bold="True" Font-Size="X-Small" Width="100px" OnClick="CalculateFormulaP" />
                                                     <asp:Button ID="btnPrint" runat="server" Visible="false" Text="Self Invoice" CssClass="btnNew" Font-Bold="True" Font-Size="X-Small" Width="100px" />
                ...
                <tr>
                    <td>
                        <asp:UpdatePanel ID="updMsg" runat="server" UpdateMode="Conditional">
                            <ContentTemplate>
                                <asp:Label ID="lblMsg" runat="server" Font-Bold="true" ForeColor="Green" Font-Size="Small"></asp:Label>&nbsp;
