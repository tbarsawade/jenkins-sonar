...
Partial Class Test2
    ...
    <WebMethod()> _
   <Script.Services.ScriptMethod()> _
    Public Shared Function GetMarkerListJSON2() As String
        'Dim ds As New DataSet()
        'ds.Tables.Add("table1")
    ...
    <WebMethod()> _
 <Script.Services.ScriptMethod()> _
    Public Shared Function GetMarkerInfo(Id As String) As String
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim apikey As String = String.Empty

        Dim query As String = "select m.tid[TID],m.fld10[SiteID],replace(replace(replace(replace(m.fld11,'(',' '),')',' '),'{',' '),'}',' ')[SiteName],"
        query &= "m.fld13[Address],m1.fld1[Site],''[Info],ltrim(rtrim(left(replace(m.fld21,',','        '),9)))[Lat] ,rtrim(ltrim(right(replace(m.fld21,',','        '),9)))[Long], m.fld12[Group],m.fld11[Site Name],''[Site Address],u.UserName[OandM Head],u1.UserName[Maintenance Head], u2.UserName[Opex Manager],u3.UserName[Security Manager],u4.UserName[Zonal Head],u5.UserName[Cluster Manager],m2.fld1[Supervisor], m3.fld1[Technician]from  mmm_mst_master m with (nolock) inner join mmm_mst_user u with (nolock) on convert(nvarchar,u.uid)=m.fld23 join mmm_mst_user u1 on convert(nvarchar,u1.uid)=m.fld24 join mmm_mst_user u2 on convert(nvarchar,u2.uid)=m.fld25  join mmm_mst_user u3 on convert(nvarchar,u3.uid)=m.fld26  join mmm_mst_user u4 on convert(nvarchar,u4.uid)=m.fld27 join mmm_mst_user u5 on convert(nvarchar,u5.uid)=m.fld28 join mmm_mst_master m2 on convert(nvarchar,m2.tid)=m.fld29 join mmm_mst_master m3 on convert(nvarchar,m3.tid)=m.fld3 join mmm_mst_master m1 with (nolock) on convert(nvarchar,m1.tid)=m.fld12  where m.documenttype='Site' and m1.documenttype='Site Type' and m1.eid=54 and m.eid=54 and  u.eid=54 and "
