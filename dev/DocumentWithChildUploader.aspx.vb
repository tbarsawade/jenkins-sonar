...
Partial Class DocumentWithChildUploader
    ...
    Public Sub BindDropDown()
        ...
        Try
            ...
            Dim MQuerry = "SELECT MID,MenuName,PageLink FROM MMM_MST_MENU WHERE MID NOT IN (SELECT DISTINCT PMENU FROM MMM_MST_MENU WHERE PMENU IS NOT NULL and EID=" & Session("EID") & ") and eid=" & Session("EID") & " AND PageLink like 'Documents.Aspx?%'and ( roles like '%{" & Session("USERROLE") & ":15}%" & "' OR roles like '%{" & Session("USERROLE") & ":11}%" & "' OR roles like '%{" & Session("USERROLE") & ":7}%" & "' OR roles like '%{" & Session("USERROLE") & ":5}%" & "' OR roles  like '%{" & Session("USERROLE") & ":3}%" & "')"
    ...
    Protected Sub btnChk_Click(sender As Object, e As EventArgs) Handles imgCSVSample.Click
        ...
        Try
            ...
            da.SelectCommand.CommandText = "select lookupvalue from mmm_mst_fields where eid=" & Session("eid") & " and documenttype='" & scrname & "' and lookupvalue like '%-R%'"
            If con.State <> ConnectionState.Open Then
                con.Open()
            End If
            da.Fill(dt)
            For i As Integer = 0 To dt.Rows.Count - 1
                Dim str() As String = Split(dt.Rows(i).Item(0).ToString, ",")
                For f As Integer = 0 To str.Length - 2
                    Dim SD As String() = Split(str(f), "-")
                    If SD(1).ToUpper = "R" Then
                        ID = ID & SD(0).ToString & ","
                    End If
                Next
            Next
            ...
            'da.SelectCommand.CommandText = "SELECT 'DocNumber' As 'DisPlayName','Yes'[Mandatory Fields],'Numeric Digits'[Data Type],'1' [Minimum Length] ,''[Maximum Length] UNION ALL SELECT rtrim(ltrim(displayName)) AS [DisplayName],case isRequired when 0 then 'No' when 1 then 'Yes' end [Mandatory Fields],case datatype when 'text' then 'Text' when 'numeric' then 'Numeric Digits' when 'Datetime' then 'Date in (DD/MM/YY)' end [Data Type],case MinLen when 0 then '' else MinLen end [Minimum Length], case MaxLen when 0 then '' else maxlen end [Maximum Length] FROM MMM_MST_FIELDS where EID =" & Session("eid") & " and DocumentType ='" & scrname & "' AND ISACTIVE=1   AND FieldType<>'ChildUploader Item' AND  FieldType <>'Formula Field' and FieldType <> 'Calculative Field' AND  IsEditable=1 and invisible=0 and fieldid not in (" & ID & ")     UNION ALL SELECT rtrim(ltrim(DocumentType))+ '.'+ rtrim(ltrim(displayName ))AS [Display Name],case isRequired when 0 then 'No' when 1 then 'Yes' end [Mandatory Fields],case datatype when 'text' then 'Text' when 'numeric' then 'Numeric Digits' when 'Datetime' then 'Date in (DD/MM/YY)' end [Data Type],case MinLen when 0 then '' else MinLen end [Minimum Length], case MaxLen when 0 then '' else maxlen end [Maximum Length] FROM MMM_MST_FIELDS where EID =" & Session("eid") & " and DocumentType in(SELECT Dropdown FROM MMM_MST_FIELDS WHERE EID=" & Session("eid") & " AND DocumentType ='" & scrname & "') AND ISACTIVE=1 and  FieldType <>'Formula Field' AND FieldType <>'ChildUploader Item' and FieldType <> 'Calculative Field' AND  IsEditable=1 and invisible=0 "
            da.SelectCommand.CommandText = "select [DisplayName],[Mandatory Fields],[Data Type],[Minimum Length],[Maximum Length],[DisplayOrder] from(SELECT 'DocNumber' As 'DisplayName','Yes'[Mandatory Fields],'Numeric Digits'[Data Type],'1' [Minimum Length] ,''[Maximum Length] ,''[DisplayOrder] UNION all SELECT rtrim(ltrim(displayName)) AS [DisplayName],case isRequired when 0 then 'No' when 1 then 'Yes' end [Mandatory Fields],case datatype when 'text' then 'Text' when 'numeric' then 'Numeric Digits' when 'Datetime' then 'Date in (DD/MM/YY)' end [Data Type],case MinLen when 0 then '' else MinLen end [Minimum Length], case MaxLen when 0 then '' else maxlen end [Maximum Length], displayorder[DisplayOrder] FROM MMM_MST_FIELDS where EID =" & Session("eid") & " and DocumentType ='" & scrname & "' AND ISACTIVE=1 AND FieldType<>'ChildUploader Item' AND  FieldType <>'Formula Field' and FieldType <> 'Calculative Field' AND  IsEditable=1 and invisible=0 and fieldid not in (" & ID & ")) as t union all select * from (SELECT rtrim(ltrim(DocumentType))+ '.'+ rtrim(ltrim(displayName ))AS [Display Name],case isRequired when 0 then 'No' when 1 then 'Yes' end [Mandatory Fields],case datatype when 'text' then 'Text' when 'numeric' then 'Numeric Digits' when 'Datetime' then 'Date in (DD/MM/YY)' end [Data Type],case MinLen when 0 then '' else MinLen end [Minimum Length], case MaxLen when 0 then '' else maxlen end [Maximum Length],200 + displayorder[DisplayOrder] FROM MMM_MST_FIELDS where EID =" & Session("eid") & " and DocumentType in(SELECT Dropdown FROM MMM_MST_FIELDS WHERE EID=" & Session("eid") & " AND DocumentType ='" & scrname & "') AND ISACTIVE=1 and  FieldType <>'Formula Field' AND FieldType <>'ChildUploader Item' and FieldType <> 'Calculative Field' AND  IsEditable=1 and invisible=0 ) as d order by displayorder"
            If con.State <> ConnectionState.Open Then
                con.Open()
            End If
            da.Fill(ds, "data")
            ds.Tables("data").Columns.Remove("DisplayOrder")
    ...
    Protected Sub btnUpload_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        Try
            ...
            Dim lstData As New List(Of DocumentUpUploader)
            Dim ObjDoc As DocumentUpUploader
            ...
            If CSVUploader.HasFile Then
                If hdnFileType.Value = "BOTH" Then
                    If Right(CSVUploader.FileName, 4).ToUpper() = ".CSV" Then
                        ...
                        Dim Query = "select DisplayName,fieldType,datatype,DropDown FROM MMM_MST_Fields where EID=" & EID & " AND  IsActive=1 and FieldType='ChildUploader Item' and documentType= '" & DocType & "'"
                        Dim ds As New DataSet()
                        ...
                        Using con = New SqlConnection(conStr)
                            ...
                            End Using
                        End Using
                        Dim lstChild As New List(Of ChildUploader)
                        Dim objC As ChildUploader
                        If ds.Tables(0).Rows.Count > 0 Then
                            For j As Integer = 0 To ds.Tables(0).Rows.Count - 1
                                objC = New ChildUploader()
                        ...
                        If dtData.Rows.Count > 0 Then
                            ...
                            For i As Integer = 0 To dtData.Rows.Count - 1
                                ...
                                If RowID <> Convert.ToInt32(dtData.Rows(i).Item("DocNumber")).ToString.Trim Then
                                    ...
                                    For Each Child As ChildUploader In lstChild
                            ...
                            For Each Document In lstData
                                ...
                                If Result.ToUpper().Contains("YOUR DOCID IS") Then
                    ...
                    If Right(CSVUploader.FileName, 4).ToUpper() = ".CSV" Then
                        ...
                        If dtData.Rows.Count > 0 Then
                            ...
                            For i As Integer = 0 To dtData.Rows.Count - 1
                                ...
                                'Dim strUnique As String = objDC.ExecuteQryScaller("SELECT 'DocNumber' As 'DisPlayName'")
                                If strUnique <> "" Then
                                            ...
                                            For Each column As DataColumn In dtData.Columns
                                                If column.ColumnName.Trim.Contains(Child.DocumentType & ".") Then
                                                    If (dtData.Rows(i).Item(column.ColumnName).ToString.Trim <> "") Then
                                                        ...
                                                        ChildStr.Append("()").Append(arr(1).Trim).Append("<>").Append(Regex.Replace(dtData.Rows(i).Item(column.ColumnName).ToString.Replace("|", " I "), "[^\u0000-\u007F]", String.Empty))
                            ...
                            For Each item In lstData
                                ...
                                If item.Response.ToUpper().Contains("ERROR(S) IN") Then
                                    Dim test = ""
                                ...
                                If arraylist(x).ToString().ToUpper.Contains(".CSV") Then
                                    ...
                                    If dtData.Rows.Count > 0 Then
                                        ...
                                        For Each Document In lstData
                                            ...
                                            Next
                                            Dim StrData As String = mainStr.ToString()
                                            Dim Result = CommanUtil.ValidateParameterByDocumentType(EID, DocType, UID, StrData, Nothing, False, False, False, arraylist, prefixFilePath:=prefixFilePath)
                                            Result = Result.Replace("Error(s) in document " & DocType & ".", String.Empty)
                                            Dim strPatt As String = "[|\d-]|"
                                            Result = Regex.Replace(Result, strPatt, String.Empty)
                                            Document.Response = Result
    ...
    Private Function GetRandomRows(ByVal table As DataTable, ByVal rowCount As Integer, ByVal restable As DataTable, ByVal str As String) As DataTable
        ...
        Return restable
    End Function
    Public Function CreateChildList(ds As DataSet) As List(Of ChildUploader)
        Dim lstChild As New List(Of ChildUploader)
        Dim objC As ChildUploader
        If ds.Tables(0).Rows.Count > 0 Then
            For j As Integer = 0 To ds.Tables(0).Rows.Count - 1
    ...
    Protected Sub GetDataFromXml(ByVal strDataFilePath As String)
        Try
            ...
            If (xmlnode.Count > 0) Then
                ...
                Dim Querys = "select inwardxmltagname,displayName from mmm_mst_fields where eid=" & EID & " and documenttype in(select dropdown from mmm_mst_fields where eid=" & EID & " and documenttype='" & ddlFormtype.SelectedItem.Text.Trim() & "' and fieldType='ChildUploader Item') and inwardxmltagname is not null;select inwardxmltagname,displayName from mmm_mst_fields where eid=" & EID & " and documenttype='" & ddlFormtype.SelectedItem.Text.Trim() & "'and fieldType<>'ChildUploader Item' and  inwardxmltagname is not null;select inwardxmltagname,dropdown from mmm_mst_fields where eid=" & EID & " and documenttype='" & ddlFormtype.SelectedItem.Text.Trim() & "'and fieldType='ChildUploader Item' and inwardxmltagname is not null"
    ...
    Public Function CreateCollecttion(ByRef LstDoc As List(Of DocumentUpUploader), DocNumber As String, ChildName As String, Datastring As String, HasData As Boolean) As Boolean
        Try
            'Dim ObjDOC = From Obj In LstDoc Where (Obj.DocNumber = DocNumber) Select Obj
            Dim obj As DetailChidUploader
    ...
    Protected Sub ddlFormtype_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        Dim Query = "select iscsv,isxml,AllowZipFile,isnull(fieldseperator,'') [fieldseperator] from mmm_mst_forms where eid=" & EID & " and FormName='" & ddlFormtype.SelectedItem.Text.Trim() & "'"
        Dim dt As New DataTable()
        'Code For populating non editable field with current date
        Using con = New SqlConnection(conStr)
            Using da = New SqlDataAdapter(Query, con)
...
End Class
Public Class ChildUploader
    Public Property DocumentType As String
    Public Property chldData As New List(Of DetailChidUploader)
End Class
Public Class DocumentUpUploader
    Public Property DocNumber As String
    Public Property DataString As String
    Public Property lstChild As New List(Of ChildUploader)
    Public Property Response As String
End Class
Public Class DetailChidUploader
