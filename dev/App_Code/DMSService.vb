...
Public Class DMSService
    ...
    Function Scheduler(ByVal FTid As Integer) As Boolean
        ...
        Try
            ...
            'Dim da As New SqlDataAdapter("select ScheduleTime from MMM_MST_ReportScheduler where tid=" & FTid, con)
    ...
    Public Function Run_FTP_Inward_Integration() As String Implements IDMSService.Run_FTP_Inward_Integration
        ...
        Try
            ...
            Call SendAlertMails()
        Catch ex As Exception
            AutoRunLog("SLA_EXPIRY_Reminders", "SAL EXPIRY Reminders", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try
        ...
        Catch ex As Exception
            AutoRunLog("DOC_EXPIRY_Reminders", "DOCUMENT EXPIRY Reminders", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try
        ...
        ''''IHCL Outward Integration Call 
        Try
            IHCLWebServiceOutwardIntegration()
        Catch ex As Exception
            AutoRunLog("IHCLWebServiceOutwardScheduler", "IHCL Integration Execution Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        ''preeti
        Try
            lenskartWebServiceOutwardIntegration()
        Catch ex As Exception
            AutoRunLog("LenskartWebServiceOutwardScheduler", "Lenskart Integration Execution Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        Try
            VLCCWebServiceOutwardIntegration()
        Catch ex As Exception
            AutoRunLog("VLCCWebServiceOutwardIntegration", "VLCC Integration Execution Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        ''''Report Staging SAVEFILEREPORTTO_STAGING
        Try
            SAVEFILEREPORTTO_STAGING()
        Catch ex As Exception
            AutoRunLog("StagingReportScheduler", "Staging Report Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        '''' Call Rental tool related main fuction balow  - 08-July-20 by Chandni
        ''''''' disabled below as no longer used as there is seperate instance of rental application
        'Try
        ...
        ''''XML Jv Integration for Ambit
        Try
            CreateXMLForAmbitJV()
        Catch ex As Exception
            AutoRunLog("AmbitXMLReportScheduler", "JV XML Report Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        ''''Auto Close of PO for PayU
        Try
            PayUAutoCloseExpPOs()
        Catch ex As Exception
            AutoRunLog("PayUAutoCloseExpPOs", "PayU AutoClose Scheduler", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        ''''Auto SES update stage approval if found in GRN document for Rockman by Preeti 22Oct2021
        Try
            RockmanAutoapproveSESstage()
        Catch ex As Exception
            AutoRunLog("RockmanAutoapproveSESstage", "Rockman Auto approve SES stage", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        End Try

        Return ""
    End Function

    Public Sub RockmanAutoapproveSESstage()
        Try
            Dim isTime As Boolean = False
            Dim cTime As Integer
            cTime = (Now.Hour * 100) + Now.Minute
            If (cTime >= 840 And cTime <= 850) Or (cTime >= 1040 And cTime <= 1050) Or (cTime >= 1240 And cTime <= 1250) Or (cTime >= 1440 And cTime <= 1450) Or (cTime >= 1640 And cTime <= 1650) Or (cTime >= 1840 And cTime <= 1850) Or (cTime >= 2040 And cTime <= 2050) Then
                isTime = True
            Else
                isTime = False
            End If
            If isTime Then
                ...
                Using con As SqlConnection = New SqlConnection(conStr)
                    Using da As New SqlDataAdapter("Auto_Stage_Approval", con)
                        da.SelectCommand.CommandType = CommandType.StoredProcedure
                        da.SelectCommand.Parameters.AddWithValue("@documenttype", "Invoice PO")
                        da.SelectCommand.Parameters.AddWithValue("@curstatus", "SES Creation")
                        da.SelectCommand.Parameters.AddWithValue("@EId", 204)
                        da.SelectCommand.Parameters.AddWithValue("@POType", "material")
                        da.Fill(ds)
                        If (ds.Tables(0).Rows.Count > 0) Then
                            Dim iColCount As Integer = ds.Tables(0).Rows.Count
                            For i As Integer = 0 To iColCount - 1
                                Dim Invoice_number As String = Convert.ToString(ds.Tables(0).Rows(i).Item("invoice no"))
                                Dim PO_Number As String = Convert.ToString(ds.Tables(0).Rows(i).Item("PO_Number"))
                                Dim PearlID As String = Convert.ToString(ds.Tables(0).Rows(i).Item("pearl_ID"))
                                Dim dataClass As New DataClass()
                                Dim dt As DataTable = dataClass.ExecuteQryDT("select  fld1 [GRN Number]  from mmm_mst_doc with (nolock) where fld6='" & PO_Number & "' and fld10='" & Invoice_number & "' and documenttype='GRN Document' and eid=204 and curstatus='Archive' ")
                                If dt.Rows.Count > 0 Then
                                    Dim GRN_NUMBER = dt.Rows(0).Item("GRN Number")
                                    Dim responseData As ResponseData = New ResponseData
                                    Dim ApIKEY = "BHSAMLIDNSH041145JAH"
                                    Dim doctype = "INVPO SES Creation App"
                                    Dim inputString As String = "Key$$" & ApIKEY & "~DOCTYPE$$" & doctype & "~ACTIONTYPE$$APPROVAL~Data$$SES Reference Num::" & GRN_NUMBER & "|SES Approval Remarks::Auto Approved by System|SES Date::" & Date.Now.ToString("dd/MM/yyyy") & " |PEARL ID::" & PearlID & ""
                                    ...
                                    'Respone.Text = responseData.Message
                                End If
                            Next
                        End If
                    End Using
    ...
    Private Sub PayUAutoCloseExpPOs()
        Try
            Dim isTime As Boolean = False
            Dim cTime As Integer
            cTime = (Now.Hour * 100) + Now.Minute
            If (cTime >= 400 And cTime <= 410) Then
                isTime = True
            Else
                isTime = False
            End If
            If isTime Then
                ...
                ' ht.Add("@uid", uid)
                Result = objDC.ExecuteProScaller("udfAutoClose_ExpiredPO_forPayU", ht)
    ...
    Public Sub CreateXMLForAmbitJV()
            ...
            If ds.Tables("rpt").Rows.Count > 0 Then
                ...
                For j As Integer = 0 To distinctDT.Rows.Count - 1
                    ...
                    For i As Integer = 0 To Bufiltertbl.Rows.Count - 1
                        ...
                        strxml += "<GeneralDate1>" & Bufiltertbl.Rows(i).Item("GeneralDate1") & "</GeneralDate1>"
                        strxml += "<GeneralDate2>" & Bufiltertbl.Rows(i).Item("GeneralDate2") & "</GeneralDate2>"
                    ...
                    sw.WriteLine(strxml)
                    sw.Close()
                    CopyfiletoInbox(ftpadd, ftpid, ftppwd, FPath, fname, cfolder, eid, Rptname, MAILTO, RptSub, msg, cc)
                    strxml = ""
                Next
    ...
    '' Prev VERSION - bkup on 11-Nov-20 by sunil/Jayant 
    '' in new version Two addl. column (congigrable) approve and view attchment are added
    ...
    '' Latest update by Jayant and sunil for Approve button and veiw attachment on 15 Nov 2020
    Public Sub SendAlertMails()
        Dim con As SqlConnection = New SqlConnection(conStr)
        Dim da As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim dtAlert As New DataTable()
        Try
            ...
            da.SelectCommand.Parameters.Clear()
            da.SelectCommand.CommandType = CommandType.Text
            da.SelectCommand.CommandText = "select t.*,E.code from MMM_MST_Template T with (nolock) inner join mmm_mst_entity E with (nolock) on t.eid=e.eid where t.action=1 order by eid,tid"  '2629,2630,2631
            '' purchase order event name,take tid
            da.Fill(dtAlert)
            If dtAlert.Rows.Count > 0 Then
                For i As Integer = 0 To dtAlert.Rows.Count - 1
                    Try
                        ...
                        Dim SubEventNm As String = dtAlert.Rows(i).Item("SubEvent").ToString
                        Dim supervisorFLD As String = dtAlert.Rows(i).Item("userfield").ToString
                        Dim AttachmentFields As String = dtAlert.Rows(i).Item("AttachmentFields").ToString
                        Dim isIncludeApproveBtn As String = dtAlert.Rows(i).Item("isIncludeApproveBtn").ToString
                        Dim Ecode As String = dtAlert.Rows(i).Item("code").ToString
                        Dim STR As String = "", MAILTO As String = "", MAILID As String = "", subject As String = "", MSGMain As String = "", cc As String = "", Bcc As String = ""
                        Dim fn As String = "", MSG As String
                        Dim DS As New DataSet
                        '' get attachment
                        Dim fieldmapping() As String = {""}
                        If Not String.IsNullOrEmpty(AttachmentFields) Then
                            Dim Attachfields() As String = AttachmentFields.Split(",")
                            fieldmapping = AttachmentFields.Split("|")
                        End If
                        ...
                        Bcc = dtAlert.Rows(i).Item("BCC").ToString()
                        STR = dtAlert.Rows(i).Item("qry").ToString()
                        '' Add tid in query to be used for approve button and view attachement by sunil on 19-Nov-20
                        If STR.Contains("SELECT  DMS.") Then
                            STR = STR.Replace("SELECT  DMS.", "SELECT  TID, DMS.")
                        ElseIf STR.Contains("SELECT  dms.[GetNameof") Then
                            STR = STR.Replace("SELECT  dms.[GetNameof", "SELECT  TID, dms.[GetNameof")
                        End If
                            ...
                            For k As Integer = 0 To dtUsers.Rows.Count - 1
                                If Not IsDBNull(dtUsers.Rows(k).Item("userid")) Then
                                    ...
                                    For m As Integer = 0 To ArrVars.Length - 1
                                        ...
                                        MailBody &= "<td> <font face=""arial, helvetica, sans-serif"" size=""2""> " & ArrVars(m).ToString & " </font> </td>"
                                    Next
                                    If isIncludeApproveBtn = "1" Then
                                        MailBody &= "<td>Action</td><td>View</td>"
                                        MailBody &= " </tr> "
                                    ...
                                    End If
                                    Dim RunningDocID As Integer = 0

                                    For j As Integer = 0 To dtQry.Rows.Count - 1
                                        RunningDocID = dtQry.Rows(j).Item("TID") '' NEW BY SP
                                        ...
                                        If isIncludeApproveBtn = "1" Then
                                            ...
                                            apprBtn = "<td align=""center"" width=""150"" height=""28"" bgcolor=""#000091"" style=""-webkit-border-radius: 5px; -moz-border-radius: 5px; border-radius: 5px; color: #ffffff; display: block;""><a href=""http://" & Ecode.ToString() & ".myndsaas.com/ActionMailApproval.aspx?Q1=" & Q1 & "&Q2= " & Q2 & "&Q3=" & Q3 & """  style=""font-size:14px; font-weight: bold; font-family: Helvetica, Arial, sans-serif; text-decoration: none; line-height:35px;width:100%; display:inline-block""><span style=""color: #FFFFFF"">APPROVE</span></a></td>"

                                            If Not String.IsNullOrEmpty(fieldmapping(0)) Or fieldmapping(0).Contains("fld") Then
                                                Dim FieldsSet As New DataSet
                                                Dim FieldsQuery As String = "select " & fieldmapping(0) & " from mmm_mst_Doc with (nolock) where  EID=" & curEid.ToString & "  and DocumentType='" & CurEventNm.ToString & "' and tid=" & RunningDocID & ""
                                                da.SelectCommand.CommandText = FieldsQuery
                                                da.Fill(FieldsSet)
                                                If FieldsSet.Tables(0).Rows.Count > 0 Then
                                                    documentPath = "https://myndsaas.com/DOCS/" + FieldsSet.Tables(0).Rows(0).Item(0).ToString()
                                                    apprBtn &= "<td><font face=""arial, helvetica, sans-serif"" size=""2""><a href=" & documentPath & "  style=""font-size:14px; font-weight: bold; font-family: Helvetica, Arial, sans-serif; text-decoration: none; line-height:35px;width:100%; display:inline-block"">View</a></font></td>"
                                                    MailBody &= apprBtn
                                                End If
                                            Else
                                                apprBtn &= "<td><font face=""arial, helvetica, sans-serif"" size=""2""><p>N/A</p></font></td>"
                                                MailBody &= apprBtn
                                                ''Next
                                            End If
                        ...
                        End If
                    Catch ex As Exception
                        AutoRunLog("SLA_EXPIRY_Reminders", "Error in EID,TID - " & dtAlert.Rows(i).Item("EID").ToString() & "," & dtAlert.Rows(i).Item("TID").ToString(), "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
                    End Try
                Next
            End If
        Catch ex As Exception
            ...
            For d As Integer = 0 To ds.Tables("rpt").Rows.Count - 1
                ...
                If ReportScheduler(ds.Tables("rpt").Rows(d).Item("tid")) = True Then
                    If sendtype.ToString.ToUpper = "EXCEL SHEET" Then
                        ...
                        ReportName = ds.Tables("rpt").Rows(d).Item("ReportName").ToString()
                        ...
                        Else
                            ...
                            If Common.Rows.Count > 0 Then
                                ...
                                Dim mailsub As String = ds.Tables("rpt").Rows(d).Item("reportsubject").ToString()
                                Dim fname As String = ""
                                If ReportName <> "Dashboard Report" And eid <> 186 Then
                                    fname = CreateCSVR(Common, FPath, eid)
                                Else
                                    fname = CreateCSVR(Common, FPath, eid, ReportName)
                                End If
                                Dim obj As New MailUtill(EID:=eid)
                                obj.SendMail(ToMail:=MAILTO, Subject:=mailsub, MailBody:=msg, CC:=CC, Attachments:=FPath + fname, BCC:=Bcc)
                                'sendMail("vishal.kumar@myndsol.com", CC, Bcc, mailsub, msg, FPath + fname)
                    ...
                    ElseIf sendtype.ToString.ToUpper = "CSV" Then
                        Dim MAILTO As String = ds.Tables("rpt").Rows(d).Item("emailto").ToString()
                        Dim CC As String = ds.Tables("rpt").Rows(d).Item("cc").ToString()
                        Dim Bcc As String = ds.Tables("rpt").Rows(d).Item("bcc").ToString()
                        Dim str As String = ds.Tables("rpt").Rows(d).Item("reportquery").ToString()
                        ...
                        Dim reporttype As String = ds.Tables("rpt").Rows(d).Item("reporttype").ToString()
                        Dim dt As String = ds.Tables("rpt").Rows(d).Item("date").ToString()
                        Dim CSV_Delimiter As String = ds.Tables("rpt").Rows(d).Item("CSV_Delimiter").ToString()
                        eid = ds.Tables("rpt").Rows(d).Item("EID").ToString()
                        ...
                        If role <> "" Then
                            ...
                            For u As Integer = 0 To ds.Tables("User").Rows.Count - 1
                                ...
                                If Common.Rows.Count > 0 Then
                                    ...
                                    Dim mailsub As String = ds.Tables("rpt").Rows(d).Item("reportsubject").ToString()
                                    Dim fname As String = ""
                                    fname = CreateCSVFile(Common, FPath, ds.Tables("User").Rows(u).Item("uid"))
                                    Dim obj As New MailUtill(EID:=eid)
                                    obj.SendMail(ToMail:=MAILTO, Subject:=mailsub, MailBody:=msg, CC:=CC, Attachments:=FPath + fname, BCC:=Bcc)
                                End If
                                MAILTO = ""
                        ...
                        Else
                            ...
                            If Common.Rows.Count > 0 Then
                                ...
                                If (CSV_Delimiter = "|") Then
                                    fname = CreateCSVFilePIPE(Common, FPath, eid)
                                Else
                                    fname = CreateCSVFile(Common, FPath, eid)
                                End If
                                Dim obj As New MailUtill(EID:=eid)
                                obj.SendMail(ToMail:=MAILTO, Subject:=mailsub, MailBody:=msg, CC:=CC, Attachments:=FPath + fname, BCC:=Bcc)
                                'sendMail("vishal.kumar@myndsol.com", CC, Bcc, mailsub, msg, FPath + fname)
    ...
    Function MultiTimeScheduler(ByVal FTid As Integer) As Boolean
        Dim b As Boolean = False
        Try
            Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
            Dim con As SqlConnection = New SqlConnection(conStr)
                    ...
                    If stringb <> "*" Then
                        For j As Integer = 0 To str_datte.Length - 1
                            ...
                            If A.Contains("-") Then
                                ...
                                If (f <= currentdate And g >= currentdate) Then
                                    For n As Integer = 0 To str_hours.Length - 1
                                        If str_mintus.Length > 1 Then
                                            Dim x As DateTime = (Convert.ToDateTime(str_hours(n) & ":" & str_mintus(n) & ":" & "00").ToShortTimeString)
                                            ' Dim x As DateTime = (Convert.ToDateTime(stringd & ":" & stringe & ":" & "00").ToShortTimeString)
                                            If x <= time2 And x >= time1 Then
                                                b = True
                                                Exit For
    ...
    '' code for report scheduler templates calling by vishal ' latest updated by Jayant and Sunil Nov-2020
    Public Sub SendDocumentExpiryMails()
        ...
        Dim dtAlert As New DataTable
        Try
            da.SelectCommand.CommandText = "select * from MMM_MST_Template with (nolock) where action=1 and subevent='DOCUMENT EXPIRY' order by eid,tid"
            da.Fill(dtAlert)
            If dtAlert.Rows.Count > 0 Then
                For i As Integer = 0 To dtAlert.Rows.Count - 1
                    Try
                        ...
                        Dim condition As String = dtAlert.Rows(i).Item("Condition").ToString()
                        Dim frequeny As Integer = Convert.ToInt32(dtAlert.Rows(i).Item("Frequency"))
                        If SchedulerOld(CurTID) = True Then
                            da.SelectCommand.CommandType = CommandType.Text
                            da.SelectCommand.Parameters.Clear()
                            If String.IsNullOrEmpty(condition) Then
                                da.SelectCommand.CommandText = "select tid [DOCID],curstatus," & WFstatus & " [EXDT] from  MMM_MST_DOC WITH(NOLOCK) where DocumentType='" & doctype & "' and EID=" & curEid
                            Else
                                da.SelectCommand.CommandText = "select tid [DOCID],curstatus," & WFstatus & " [EXDT] from  MMM_MST_DOC WITH(NOLOCK) where DocumentType='" & doctype & "' and EID=" & curEid & " and  " & condition '' 
                            End If
                            ...
                            For j As Integer = 0 To dtDoc.Rows.Count - 1
                                ...
                                Dim dtAlert1 As New DataTable
                                da.SelectCommand.CommandType = CommandType.Text
                                da.SelectCommand.Parameters.Clear()
                                da.SelectCommand.CommandText = "select top 1 CONVERT(VARCHAR(10),AlertTime, 101)AlertTime  from MMM_LOG_ALERT where DocID='" & docID & "' And EID='" & curEid & "' order by Logid desc"
                                da.Fill(dtAlert1)
                                ...
                                If Not IsDBNull(dtDoc.Rows(j).Item("EXDT")) Then
                                    If Len(dtDoc.Rows(j).Item("EXDT")) > 5 Then
                                        ...
                                        End If
                                        ExDate = clubVals
                                        ''ExDate = "09/29/2020" '' mm/dd/yyyy 

                                        A1 = ((24) - ((DateDiff(DateInterval.Hour, Now(), ExDate)) + 24))
                                        A2 = ((24) - ((DateDiff(DateInterval.Hour, Now(), ExDate)) + 24))

                                        '' 57810>0 And 57810<99999 BSLA=0 and ASLA=999999                                    
                                        ' change by sunil p on 09-nov-20
                                        If (frequeny <= 1) Or (dtAlert1.Rows.Count <= 0) Then  '' normal previous without freq.
                                            If A1 > Bsla And A2 < Asla Then
                                                Call TemplateCalling(docID, curEid, doctype, subevent)
                                            End If
                                        Else ' if freq > 1 
                                            If A1 > Bsla And A2 < Asla Then
                                                AlertTime = dtAlert1.Rows(0).Item("AlertTime")
                                                Dim p1 = DateDiff(DateInterval.Day, Convert.ToDateTime(AlertTime), Now())
                                                If p1 = frequeny And frequeny > 1 Then
                                                    Call TemplateCalling(docID, curEid, doctype, subevent)
                                                End If
                                            End If
                                        End If
                        ...
                        End If
                    Catch ex As Exception
                        AutoRunLog("DOC_EXPIRY_Reminders", "Error in EID,TID - " & dtAlert.Rows(i).Item("EID").ToString() & "," & dtAlert.Rows(i).Item("TID").ToString(), "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
                    End Try
                Next
            End If
        Catch ex As Exception
            AutoRunLog("DOC_EXPIRY_Reminders", "General Error ", "TC Exception msg -" & Regex.Replace(ex.Message.ToString, "[""']", String.Empty), 0)
        Finally
            con.Close()
            da.Dispose()
            dtAlert.Dispose()
    ...
    Private Function CreateCSVR(ByVal dt As System.Data.DataTable, ByVal path As String, ByVal uid As String, Optional ByVal ReportName As String = "") As String
        Dim fname As String = ReportName & Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Millisecond & uid & ".xls"
    ...
    Private Function CreateCSVFile(ByVal dt As System.Data.DataTable, ByVal path As String, ByVal uid As String) As String
        'Dim fname As String = Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Second & Now.Millisecond & ".CSV"
        Try
            Dim fname As String = Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Millisecond & uid & ".csv"
            If File.Exists(path & fname) Then
                File.Delete(path & fname)
            End If
    ...
    Private Function CreateCSVFilePIPE(ByVal dt As System.Data.DataTable, ByVal path As String, ByVal uid As String) As String
        'Dim fname As String = Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Second & Now.Millisecond & ".CSV"
        Try
            Dim fname As String = Now.Year & Now.Month & Now.Day & Now.Hour & Now.Minute & Now.Millisecond & uid & ".csv"
            If File.Exists(path & fname) Then
    ...
    ' Lenskart WebService Outward Integration Process Method
    Sub lenskartWebServiceOutwardIntegration()
        Dim LenskartWebService As New LenskartWebService()
        LenskartWebService.EntryPoint()
    End Sub


    ' VLCC WebService Outward Integration Process Method
    Sub VLCCWebServiceOutwardIntegration()
        Dim VLCCWebService As New VLCCWebService()
        VLCCWebService.EntryPoint()
    End Sub

    ' IHCL WebService Outward Integration Process Method
    Sub IHCLWebServiceOutwardIntegration()
    ...
    Sub FTPREPORT()
        ...
        Try
            ...
            Dim FPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/MailAttach/")
            'oda.SelectCommand.CommandText = "select * from  MMM_MST_ReportScheduler r join mmm_mst_entity e on e.eid=r.eid  where ftpflag=2 order by hh,mm,ordering"
            oda.SelectCommand.CommandText = "select r.tid,r.eid,e.ftpaddress,e.ftpID,e.ftpPwd,e.sftpPort,r.IsSendAttachment,r.RSFolder,r.reportquery,e.ftpfolder,r.msgbody,r.emailto,r.cc,r.bcc,e.FTPType,r.sendtype,r.SendTo,r.FileSeparator,r.PostFixType,r.FileExtension,r.reportname,r.reportSubject,r.LastScheduledDate from  MMM_MST_ReportScheduler r join mmm_mst_entity e on e.eid=r.eid  where ftpflag=2 order by hh,mm,ordering"
            oda.SelectCommand.CommandType = CommandType.Text
            oda.Fill(ds, "rpt")
            For d As Integer = 0 To ds.Tables("rpt").Rows.Count - 1
                Try
                    If ReportScheduler(ds.Tables("rpt").Rows(d).Item("tid")) = True Then
                        If ds.Tables("rpt").Rows(d).Item("sendto").ToString.ToUpper = "USER" Then
                            ...
                            Dim ftpadd As String = ds.Tables("rpt").Rows(d).Item("FtpAddress").ToString()
                            Dim ftpid As String = ds.Tables("rpt").Rows(d).Item("FtpID").ToString()
                            Dim ftppwd As String = ds.Tables("rpt").Rows(d).Item("FtpPwd").ToString()
                            Dim sftpPort As String = ds.Tables("rpt").Rows(d).Item("sftpPort").ToString()
                            If sftpPort = "" Then sftpPort = "22"
                            Dim cfolder As String = ""
                            IsSendAttachment = ds.Tables("rpt").Rows(d).Item("IsSendAttachment").ToString()
                            If ds.Tables("rpt").Rows(d).Item("RSFolder").ToString() = "" Then
                                cfolder = ds.Tables("rpt").Rows(d).Item("FtpFolder").ToString()
                            Else
                            ...
                            FTPType = ds.Tables("rpt").Rows(d).Item("FTPType").ToString()
                            ...
                            Dim FileSeparator As String = ds.Tables("rpt").Rows(d).Item("FileSeparator").ToString()
                            Dim PostFixType As String = ds.Tables("rpt").Rows(d).Item("PostFixType").ToString()
                            Dim FileExtension As String = ds.Tables("rpt").Rows(d).Item("FileExtension").ToString()
                            ...
                            If FTPR.Rows.Count > 0 Then
                                ...
                                If IsSendAttachment.ToString.Trim = "1" Then
                                    OutwardAttachmentFile(ReportTid)
                                ElseIf IsSendAttachment.ToString.Trim = "2" Then
                                    OutwardAttachmentMultiDocType(ReportTid)
                                End If
                                ...
                                If filetype.ToString.ToString = "XML" Then
                                    ...
                                    If FTPType.ToUpper = "SFTP" Then
                                        CopyfiletoSFTP(ftpadd, ftpid, ftppwd, FPath, fname, cfolder, eid, Rptname, MAILTO, RptSub, msg, CC, sftpPort)
                                    Else
                                        CopyfiletoInbox(ftpadd, ftpid, ftppwd, FPath, fname, cfolder, eid, Rptname, MAILTO, RptSub, msg, CC)
                                    End If
                                End If
                                If filetype.ToString.ToString = "EXCEL" Then
                                    fname = CreateCSVFTP(FTPR, Rptname, FileExtension, PostFixType, FileSeparator)
                                ...
                                oda.SelectCommand.Parameters.Clear()
                                oda.SelectCommand.Parameters.AddWithValue("@MAILTO", "FTP")
                                oda.SelectCommand.Parameters.AddWithValue("@CC", fname)
                                oda.SelectCommand.Parameters.AddWithValue("@MSG", msg)
                                ...
                                oda.SelectCommand.Parameters.AddWithValue("@RSID", ds.Tables("rpt").Rows(d).Item("tid").ToString)
                                oda.SelectCommand.Parameters.AddWithValue("@Dcnt", CNT)
                                If con.State <> ConnectionState.Open Then
                                    con.Open()
                                End If
                                oda.SelectCommand.ExecuteNonQuery()
                ...
                Catch ex As Exception
                    Dim obj As New MailUtill(EID:=eid)
                    Try
                        If GetLastScheduledFileName(Scfname, eid) = False Then
    ...
    Public Function GetLastScheduledFileName(ByVal FileName As String, ByVal Eid As Integer) As Boolean
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim dt As New DataTable()
        Dim b As Boolean
        Try
            oda.SelectCommand.CommandText = "select count(*) from FTPFileLogs with(nolock) where eid= " & Eid & " and fname='" & FileName & "'"
            oda.SelectCommand.CommandType = CommandType.Text
            oda.Fill(dt)
        ...
        End Try
    End Function
    Private Function CreateTextFileFTP(ByVal dt As DataTable, ByVal ReportName As String, Optional ByVal FileExtension As String = "", Optional ByVal PostFixType As String = "", Optional ByVal FileSeparator As String = "") As String
    ...
    Sub SAVEFILEREPORTTO_STAGING()
        'Dim conStr As String = "server=MYNDHOSTDBVIP1;initial catalog=DMS;uid=DMS;pwd=MyN$d!5H#"
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        ...
        Try
            Dim FPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/MailAttach/")
            oda.SelectCommand.CommandText = "select * from  MMM_MST_ReportScheduler with (nolock) where ftpflag=3 order by hh,mm,ordering"
            oda.SelectCommand.CommandType = CommandType.Text
            oda.Fill(ds, "rpt")
                        ...
                        If ReportScheduler(ds.Tables("rpt").Rows(d).Item("tid")) = True Then
                            Try
                                If ds.Tables("rpt").Rows(d).Item("sendto").ToString.ToUpper = "USER" Then
                                    ...
                                    lastschedule = Format(Convert.ToDateTime(lastschedule.ToString), "yyyy-MM-dd HH:mm:ss:fff")
                                    str = Replace(str, "@lastsch", lastschedule)
                                    ''str = Replace(str, "select ROW_NUMBER()", "select top 5 ROW_NUMBER()")
                                    ...
                                    Try
                                        If FTPR.Rows.Count > 0 Then
                                            ...
                                            If MyCon.State = ConnectionState.Open Then
                                                ...
                                                If tableAction.ToString.ToUpper = "UPDATE" Then
                                                    ...
                                                    For Each row2 As DataRow In STAGING_Tbl.Rows
                                                        ...
                                                        col = ""
                                                        Value = ""
                                                        Dim aff As Integer = objCmd.ExecuteNonQuery()
                                                    Next
                                                    ''After successful insertion of data into staging table-- update MMM_MST_ReportScheduler in LastScheduledDate =Now() where tid=tid
                                                    ...
                                                    End Using
                                                Else
                                                    'LabelMsg.Text &= " inside Insert condition 171"
                                                    Dim STAGING_Tbl As New DataTable
                                                    STAGING_Tbl = FTPR
                                                    Dim Result As String = ""
    ...
    Protected Function Save_Log(ByVal CID As String, ByVal Filename As String, ByVal ActionCode As String, ByVal Empcode As String, ByVal Result As String, ByVal Remarks As String, ByVal fileRundate As String, ByVal SuccessfulRun As String, ByVal TotalAttempts As Integer, ByVal TransType As String, ByVal xmlFileNm As String, Optional ByVal Rerunstr As String = "", Optional ByVal ErrMsg As String = "", Optional ByRef LastInsertDate As String = "") As String
        ...
        Save_Log = ""
    End Function
    Public Sub UpdateLastScheduleDate(ByVal tid As Integer, ByVal eid As Integer, con As SqlConnection, ByVal alertname As String)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
    ...
    Protected Sub CopyfiletoInbox(ByVal Fadd As String, ByVal login As String, ByVal pwd As String, ByVal readPath As String, ByVal filenm As String, ByVal cfoldernm As String, ByVal eid As String, ByVal RptName As String, ByVal MAILTO As String, ByVal RptSub As String, ByVal msg As String, ByVal CC As String)
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Try
    ...
    Public Function SendStagingIntegrationLogMail(ByVal CurCid As String, ByVal tid As Integer, ByVal LastRunDMY As String) As String
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim da As New SqlDataAdapter("", con)
        Try
    ...
    Private Function CreateCSV(ByVal dt As DataTable, ByVal apid As String) As String
        ...
        End Try
    End Function
    Private Function CheckIfFileExistsOnServer(ByVal URI As String, ByVal login As String, ByVal pwd As String) As Boolean
        ...
        Return False
    End Function
    Protected Sub CopyfiletoSFTP(ByVal Fadd As String, ByVal login As String, ByVal pwd As String, ByVal readPath As String, ByVal filenm As String, ByVal cfoldernm As String, ByVal eid As String, ByVal RptName As String, ByVal MAILTO As String, ByVal RptSub As String, ByVal msg As String, ByVal CC As String, Optional ByVal Port As String = "22")
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim FPath As String = ""
        Try
            ' Dim URI As String = Fadd & "/" & filenm
            FPath = System.Web.Hosting.HostingEnvironment.MapPath("~/MailAttach/" & filenm)

            'prev without port
            'Dim client As SftpClient = New SftpClient(Fadd, login, pwd)
            Dim client As SftpClient = New SftpClient(Fadd, Port, login, pwd)
            client.Connect()
            ...
            If client.Exists(cfoldernm & filenm) = True Then
                oda.SelectCommand.CommandText = "INSERT_FILELOG"
                oda.SelectCommand.CommandType = CommandType.StoredProcedure
                oda.SelectCommand.Parameters.Clear()
                oda.SelectCommand.Parameters.AddWithValue("@EID", eid)
                oda.SelectCommand.Parameters.AddWithValue("@FNAME", filenm)
                oda.SelectCommand.Parameters.AddWithValue("@ReportName", RptName)
                oda.SelectCommand.Parameters.AddWithValue("@FileType", "SFTP")
                If con.State <> ConnectionState.Open Then
                    con.Open()
                End If
                oda.SelectCommand.ExecuteNonQuery()
        ...
        Catch ex As Exception
            Dim obj As New MailUtill(EID:=eid)
            If MAILTO.ToString.Trim <> "" Then
                obj.SendMail(ToMail:=MAILTO, Subject:=RptSub & " with filename (" & filenm & " ) Integration Failed", MailBody:=msg, CC:=CC, Attachments:=FPath, BCC:="")
            End If
    ...
    Private Function CreateCSVFTP(ByVal dt As DataTable, ByVal ReportName As String, Optional ByVal FileExtension As String = "", Optional ByVal PostFixType As String = "", Optional ByVal FileSeparator As String = "") As String
        Dim month As String = Now.Month.ToString()
        Dim day As String = Now.Day.ToString()
        Dim milisec As String = Now.Millisecond.ToString()
        ...
        Else
            If FileExtension = "" Then
                If PostFixType = "" Or PostFixType.ToUpper = "DATETIME" Then
                ...
                Else
                    fname = ReportName & "_" & Now.Year & month.PadLeft(2, "0") & day.PadLeft(2, "0") & ".CSV"
                End If

            Else
                If PostFixType = "" Or PostFixType.ToUpper = "DATETIME" Then
                    fname = ReportName & "_" & Now.Year & month.PadLeft(2, "0") & day.PadLeft(2, "0") & Now.Hour.ToString.PadLeft(2, "0") & Now.Minute.ToString.PadLeft(2, "0") & Now.Second.ToString.PadLeft(2, "0") & milisec.ToString.PadLeft(2, "0") & FileExtension
                Else
                    fname = ReportName & "_" & Now.Year & month.PadLeft(2, "0") & day.PadLeft(2, "0") & FileExtension
                End If
            End If
    ...
    Sub OutwardAttachmentFile(ByVal Tid As String)
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim ds As New DataSet()
        ...
        Try
            ...
            If ds.Tables("rpt").Rows.Count > 0 Then
                ...
                ' If ds.Tables("rpt").Rows(d).Item("sendto").ToString.ToUpper = "USER" Then
                eid = ds.Tables("rpt").Rows(0).Item("eid").ToString()
                msg = ds.Tables("rpt").Rows(0).Item("msgbody").ToString()
                MAILTO = ds.Tables("rpt").Rows(0).Item("emailto").ToString()
                CC = ds.Tables("rpt").Rows(0).Item("cc").ToString()
                Bcc = ds.Tables("rpt").Rows(0).Item("bcc").ToString()
                RptSub = ds.Tables("rpt").Rows(0).Item("reportsubject").ToString()
                ...
                Dim lastschedule As String = ds.Tables("rpt").Rows(0).Item("LastScheduledDate").ToString
                Dim PrefixName As String = ds.Tables("rpt").Rows(0).Item("PreFixAttachedName").ToString
                ' lastschedule = Format(Convert.ToDateTime(lastschedule.ToString), "yyyy-MM-dd HH:mm:ss:fff")
                'str = Replace(str, "@lastsch", lastschedule)
                oda.SelectCommand.CommandText = str
                oda.SelectCommand.CommandType = CommandType.Text
                ...
                For i As Integer = 0 To FTPR.Rows.Count - 1
                    Try
                        ...
                        If (stringSpiltArray IsNot Nothing AndAlso stringSpiltArray.Count > 0) Then
                            For Each strField As String In stringSpiltArray
                                ...
                                If PrefixName.Length > 1 Then
                                    RNameFileName = PrefixName & "_" & RNameFileName
                                End If
                                If stringSpiltArray.Count > 1 Then
                                    oda.SelectCommand.CommandText = "select Displayname from  MMM_MST_fields where eid='" & eid & "' and DocumentType= '" & DocType & "' And fieldmapping ='" & strField & "'"
                                    oda.SelectCommand.CommandType = CommandType.Text
                                    oda.Fill(ds, "Dispname")
                                    RNameFileName = RNameFileName & "_" & ds.Tables("Dispname").Rows(0).Item(0).ToString()
                                    ds.Tables("Dispname").Clear()
                                End If
                                ...
                                If Not String.IsNullOrEmpty(FileName) Then
                                    ...
                                    If System.IO.File.Exists(copyFile) Then
                                    ...
                                    If (FTPType.ToUpper = "SFTP") Then
                                        CopyfiletoSFTP(ftpadd, ftpid, ftppwd, FPath, FtpFile, cfolder, eid, Rptname, MAILTO, RptSub, msg, CC)
                                    Else
                                        CopyfiletoInbox(ftpadd, ftpid, ftppwd, FPath, FtpFile, cfolder, eid, Rptname, MAILTO, RptSub, msg, CC)
                                    End If
                    ...
                    Catch ex As Exception
                        ...
                        oda.SelectCommand.Parameters.Clear()
                        oda.SelectCommand.Parameters.AddWithValue("@ERRORMSG", ex.ToString())
                        oda.SelectCommand.Parameters.AddWithValue("@ALERTNAME", "Outward File Attachment")
                        oda.SelectCommand.Parameters.AddWithValue("@EID", eid)
                        If con.State <> ConnectionState.Open Then
                            con.Open()
                        End If
        ...
        Catch ex As Exception
            ...
            If MAILTO.ToString <> "" Then
                obj.SendMail(ToMail:=MAILTO, Subject:=RptSub & " Attachment File Issue", MailBody:=msg, CC:=CC, Attachments:="", BCC:=Bcc)
            End If
            oda.SelectCommand.CommandText = "INSERT_ERRORLOG"
            oda.SelectCommand.CommandType = CommandType.StoredProcedure
            oda.SelectCommand.Parameters.Clear()
    ...
    Sub OutwardAttachmentMultiDocType(ByVal Tid As String)
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim ds As New DataSet()
        ...
        Try
            Dim FPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/MailAttach/")
            Dim DocPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/DOCS/")
            oda.SelectCommand.CommandText = "select * from  MMM_MST_ReportScheduler where isSendAttachment=2 and Tid='" & Tid & "'"
            oda.SelectCommand.CommandType = CommandType.Text
            oda.Fill(ds, "rpt")
            If ds.Tables("rpt").Rows.Count > 0 Then
                eid = ds.Tables("rpt").Rows(0).Item("eid").ToString()
                ...
                For i As Integer = 0 To FTPR.Rows.Count - 1
                    Try
                        Dim copyFile As String = ""
                        Dim FtpFile As String = ""
                        Dim FileName As String = FTPR.Rows(i).Item("Fieldstosend").ToString()
                        Dim Docid As String = FTPR.Rows(i).Item("DOCID").ToString()
                        Dim PrefixName As String = FTPR.Rows(i).Item("PrefixName").ToString()
                        Dim RNameFileName As String = FTPR.Rows(i).Item("AttRefield").ToString()
                        Dim DocumentType As String = FTPR.Rows(i).Item("DocumentType").ToString()
                        If PrefixName.Length > 1 Then
                            RNameFileName = PrefixName & "_" & RNameFileName
                        End If
                        RNameFileName = RNameFileName.Replace("/", "_")
                        RNameFileName = RNameFileName.Replace("\", "_")
                        If Not String.IsNullOrEmpty(FileName) Then
                            Dim strArr() As String
                            strArr = FileName.Split("/")
                    ...
                    Catch ex As Exception
                        ...
                        oda.SelectCommand.Parameters.Clear()
                        oda.SelectCommand.Parameters.AddWithValue("@ERRORMSG", ex.ToString())
                        oda.SelectCommand.Parameters.AddWithValue("@ALERTNAME", "Outward Mutliple File Attachment")
                        oda.SelectCommand.Parameters.AddWithValue("@EID", eid)
                        If con.State <> ConnectionState.Open Then
                            con.Open()
                        End If
...
Public Class Hcl_apprvl_pending
    <DataMember(Name:="Bpmid", Order:=1)>
    Public Property Bpmid As String
    <DataMember(Name:="Invoice_Type", Order:=2)>
    Public Property Invoice_Type As String
    <DataMember(Name:="Vendor_Name", Order:=3)>
    Public Property Vendor_Name As String
    <DataMember(Name:="Invoice_No", Order:=4)>
    Public Property Invoice_No As String
    <DataMember(Name:="Invoice_Amount_WO_Tax", Order:=5)>
    Public Property Invoice_Amount_WO_Tax As String
    <DataMember(Name:="Pending_Days", Order:=6)>
    Public Property Pending_Days As String
    <DataMember(Name:="Creation_Date", Order:=7)>
    Public Property Creation_Date As String
    <DataMember(Name:="L1_Approver", Order:=8)>
    Public Property L1_Approver As String
    <DataMember(Name:="L2_Approver", Order:=9)>
    Public Property L2_Approver As String
    <DataMember(Name:="L3_Approver", Order:=10)>
    Public Property L3_Approver As String
    <DataMember(Name:="L4_Approver", Order:=11)>
    Public Property L4_Approver As String
    <DataMember(Name:="Ecode", Order:=12)>
    Public Property Ecode As String
    <DataMember(Name:="Emp_Name", Order:=13)>
    Public Property Emp_Name As String
    <DataMember(Name:="Status_In_Pearl", Order:=14)>
