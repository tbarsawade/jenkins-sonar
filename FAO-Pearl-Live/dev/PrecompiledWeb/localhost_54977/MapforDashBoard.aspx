<%@ page language="VB" autoeventwireup="false" inherits="MapforDashBoard, App_Web_pnyzbdje" viewStateEncryptionMode="Always" %>
<!DOCTYPE html>
   ...
   <title></title>
    ...
    <script type="text/javascript">
        ...
        function MyndMap() {
            $.ajax({
                type: "post",
                url: "MapforDashBoard.aspx/ConvertDataTabletoString",
                data: "{}",
    ...
    <style type="text/css">
#mask { position: fixed;top: 0;left: 0;right: 0;bottom: 0;background-color: #F6F6F6;z-index: 10000;height: 100%; display:none; opacity:0.9; }
#loader {position: absolute;left: 50%;top: 50%;background-image: url("images/uploading.gif");background-repeat: no-repeat;background-position: center;margin: -100px 0 0 -100px; display:none; padding-top:25px; }
</style>
    <script type="text/javascript">
        initialize();
        ...
        function bindMap(boundary) {
           ...
           var mapProp = {
            ...
            for (i = 0; i < locations.length-1; i++) {
                marker = new google.maps.Marker({ position: new google.maps.LatLng(locations[i][1], locations[i][2]), map: map, icon: locations[i][3], size: new google.maps.Size(2, 2) });
                google.maps.event.addListener(marker, 'click', (function (marker, i) {
                    return function () {
                        infowindow.setContent(locations[i][0]);
                        infowindow.open(map, marker);
