        ...
        for (i = 0; i < myMarkersArray.length - 1; i++) {
            ...
            if (IsSiteId) {
                if (myMarkersArray[i].primaryKey.toUpperCase().match(txt.toUpperCase())) {
                    var row = '<tr id="' + myMarkersArray[i].primaryKey + '" onclick="GmapTest.FocusSite(this);"><td>' + myMarkersArray[i].primaryKey + '<td/><tr/>';
                    $(body).append(row);
                }
            }
            else {
                if (myMarkersArray[i].SiteName.toUpperCase().match(txt.toUpperCase())) {
                    var row = '<tr id="' + myMarkersArray[i].primaryKey + '" onclick="GmapTest.FocusSite(this);"><td>' + myMarkersArray[i].SiteName + '<td/><tr/>';
                    $(body).append(row);
                }
            }
        }
...
GmapClass.prototype.GetMarkers = function (ddlsite, ddlVechical) {
    ...
    $.ajax({
        type: "POST",
        url: "GMapHomeTest.aspx/GetMarkerListCSV",
        data: '{IDs: "' + str + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: GmapTest.DrawMarkers,
...
GmapClass.prototype.DrawMap = function (str) {
    ...
    for (i = 0; i < locations.length - 1; i++) {
        ...
        google.maps.event.addListener(marker, 'click', (function (marker, i) {
            ...
            return function () {
                ...
                $.ajax({
                    type: "post",
                    url: "GMapHomeTest.aspx/GetMarkerInfo",
    ...
    setInterval(GmapTest.GetNewMarker, refTime);
...
GmapClass.prototype.GetNewMarker = function () {
    ...
    //url: "GMapHomeTest.aspx/RefreshVechicals",
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: GmapTest.GetNewMarkerSucceed,
        failure: function (response) {
            alert(response.d);
        }
    });
