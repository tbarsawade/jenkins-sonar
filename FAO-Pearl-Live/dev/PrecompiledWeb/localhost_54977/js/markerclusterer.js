 ...
 * =======================
 * The standard library noted above has been  modified to work with the HERE Maps API
 *
 */
 ...
 */
function MarkerClusterer(opt_markers) {
	nokia.maps.map.component.Component.call(this);
	this.init(opt_markers);
}

extend(MarkerClusterer, nokia.maps.map.component.Component);

MarkerClusterer.prototype.init = function(opt_markers) {

	var that = this;
	function ExtendedOList() {
		nokia.maps.util.OList.call(this);
	}

	extend(ExtendedOList, nokia.maps.util.OList);

	ExtendedOList.prototype.addAll = function(elements) {
		nokia.maps.util.OList.prototype.addAll.call(this, elements);
		addMarkers(elements);
	}
	ExtendedOList.prototype.add = function(element) {
		nokia.maps.util.OList.prototype.add.call(this, element);
		addMarker(element);
	}
	ExtendedOList.prototype.clear = function() {
		nokia.maps.util.OList.prototype.clear.call(this);
		clearMarkers();
	}
	ExtendedOList.prototype.remove = function(element) {
		nokia.maps.util.OList.prototype.remove.call(this, element);
		removeMarker(element)
		}
	ExtendedOList.prototype.removeAt = function(idx) {
		nokia.maps.util.OList.prototype.removeAt.call(this, idx);
		removeMarker(ExtendedOList.prototype.get.call(this, idx));
	}
	ExtendedOList.prototype.removeAll = function(elements) {
		nokia.maps.util.OList.prototype.removeAll.call(this, elements);
		var tempMap = that.map_ ;
		that.map_  = undefined;
		var len = elements.length;
		while (len--){
		//for (var i = 0; i < elements.length;++i) {
			removeMarker(elements[len]);
		//}
		}
		that.map_ = tempMap;
		
	}
	that.objects = new ExtendedOList();
   ...
   */
	function isMarkerInViewport_(marker) {
		return (that.map_ !== undefined) && that.map_.getViewBounds().contains(marker.getBoundingBox());
	}
   ...
   */
	var addMarker = function(marker, opt_isNodraw, opt_isAdded, opt_clusters, opt_isNoCheck) {
		...
		var pos = that.map_.geoToPixel(marker.coordinate);
		...
		for (var i = length - 1; i >= 0; i--) {
			...
			center = that.map_.geoToPixel(center);
		...
		cluster = new Cluster(that, that.map_);
		cluster.addMarker({
			'isAdded': isAdded,
			'marker': marker
		});
   ...
   */
	var getClustersInViewport_ = function() {
		var clusters = [];
		if (that.map_ !== undefined) {

			var curBounds = that.map_.getViewBounds();
   ...
   */
	that.resetViewport = function() {
		...
		for (var i = 0; i < clusters.length;++i) {
			...
			var curZoom = that.map_.zoomLevel;
			if (curZoom !== oldZoom) {
...
MarkerClusterer.prototype.attach = function(map) {
	this.map_ = map;
	map.addListener("mapviewchangeend", this.resetViewport);
	map.addObserver("zoomLevel", this.resetViewport);
	this.resetViewport();
};

MarkerClusterer.prototype.detach = function() {
	map.removeListener("mapviewchangeend", this.resetViewport);
	map.removeObserver("zoomLevel", this.resetViewport);
	this.map_ = null;
	this.resetViewport();
};
...
function Cluster(markerClusterer, map) {
   ...
   */
	...
	var iconSVG = ['<svg width="51" height="51" xmlns="http://www.w3.org/2000/svg">' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="25" cy="25" r="25" stroke-width="2" />' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="25" cy="25" r="21" stroke-width="2"/>' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="25" cy="25" r="17" stroke-width="2"/>' + '<text x="25" y="29" font-size="10pt" font-family="arial" font-weight="bold" text-anchor="middle" fill="#FFF" textContent="__TEXTCONTENT__">__TEXT__</text>' + '</svg>', '<svg width="47" height="47" xmlns="http://www.w3.org/2000/svg">' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="23" cy="23" r="23" stroke-width="2" />' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="23" cy="23" r="19" stroke-width="2"/>' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="23" cy="23" r="15" stroke-width="2"/>' + '<text x="23" y="27" font-size="10pt" font-family="arial" font-weight="bold" text-anchor="middle" fill="#FFF" textContent="__TEXTCONTENT__">__TEXT__</text>' + '</svg>', '<svg width="43" height="43" xmlns="http://www.w3.org/2000/svg">' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="21" cy="21" r="21" stroke-width="2" />' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="21" cy="21" r="17" stroke-width="2"/>' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="21" cy="21" r="13" stroke-width="2"/>' + '<text x="21" y="25" font-size="10pt" font-family="arial" font-weight="bold" text-anchor="middle" fill="#FFF" textContent="__TEXTCONTENT__">__TEXT__</text>' + '</svg>', '<svg width="39" height="39" xmlns="http://www.w3.org/2000/svg">' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="19" cy="19" r="19" stroke-width="2" />' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="19" cy="19" r="15" stroke-width="2"/>' + '<circle stroke="#FFF" fill="__MAINCOLOR__" cx="19" cy="19" r="11" stroke-width="2"/>' + '<text x="19" y="23" font-size="10pt" font-family="arial" font-weight="bold" text-anchor="middle" fill="#FFF" textContent="__TEXTCONTENT__">__TEXT__</text>' + '</svg>'];
	...
	createIcon = function(count, mainColor) {
		...
		var svg = iconSVG[digit].replace(/__TEXTCONTENT__/g, count).replace(/__TEXT__/g, count).replace(/__MAINCOLOR__/g, color);
