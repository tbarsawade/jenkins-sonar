    ...
    Protected Function Maprendorforsite(ByVal sid As String) As String
        Try
            ...
            For i As Integer = 0 To dtImieNo.Rows.Count - 1
                Dim str As String = "select convert(nvarchar,g.tid)[TID],IMIENO,max(m2.fld10)[Site_Name],DMS.IdealTime(g.imieno) IdealTime,convert(nvarchar,Lattitude)[Lat],convert(nvarchar,longitude)[Long],convert(nvarchar,ctime,101)ctime,max(m2.fld14)[Group],0 as Isvehicle, max(m2.fld1)[VhNo] "
                str = str & " from mmm_mst_gpsdata g with (nolock) join mmm_mst_master m2 with (nolock) on m2.fld12=g.imieno   where m2.documenttype='vehicle' and m2.fld14 in (" & sid & ")  and m2.eid=" & Session("Eid") & " group by g.tid,imieno,cTime,lattitude,longitude having ctime=(select max(ctime) from mmm_mst_gpsdata with (nolock) where imieno=g.imieno) and Ltrim(IMIENO) <> '' and  IMIENO='" & dtImieNo.Rows(i).Item("IMIENO") & "'"
                oda.SelectCommand.CommandText = str
                oda.SelectCommand.CommandTimeout = 180
                oda.Fill(dtVechical)
            Next
            For i As Integer = 0 To dtVechical.Rows.Count - 1
                If Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) > 10 And Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) <= 600 Then ''And Convert.ToDateTime(dtVechical.Rows(i).Item("ctime")) = DateAndTime.Today() Then
                ...
                ElseIf Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) >= 0 And Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) <= 10 Then ''And Convert.ToDateTime(dtVechical.Rows(i).Item("ctime")) = DateAndTime.Today() Then
                    ...
                    LstVehicle.Items(j).Attributes.Add("onclick", "focusMarker(this);")
                    j = j + 1
                ElseIf (Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) > 600 And Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) <= 1440) Then ''Or (Convert.ToDateTime(dtVechical.Rows(i).Item("ctime")) = DateAndTime.Today() And Convert.ToInt32(dtVechical.Rows(i).Item("IdealTime").ToString) > 1440) Then
                    LstVehicle.Items.Add(dtVechical.Rows(i).Item("VhNo").ToString())
                    LstVehicle.Items(j).Text = i + 1 & " " & dtVechical.Rows(i).Item("Site_Name").ToString()
                    LstVehicle.Items(j).Value = dtVechical.Rows(i).Item("IMIENO").ToString()
...
<Script.Services.ScriptMethod()> _
    Public Shared Function GetMarkerListJSON2(IDs As String) As String
        ...
        Dim d1 As String = GetSites(IO.File.ReadAllText(CsvPath))
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As SqlConnection = New SqlConnection(conStr)
        ...
        End If
        qry &= " where Eid=" & HttpContext.Current.Session("Eid") & " and Documenttype='Manpower Master' and m.fld1<>'0'"
        da.SelectCommand.CommandText = qry
        da.Fill(dt)
        For i As Integer = 0 To dt.Rows.Count - 1
            da.SelectCommand.CommandText = "Select top 1 Lattitude, Longitude from mmm_mst_gpsdata with(nolock) where IMIENO='" & dt.Rows(i).Item("UserID") & "' and datediff(hh,ctime, getdate())<=2 order by ctime desc"
 ...
 <Script.Services.ScriptMethod()> _
    Public Shared Function GetMarkerInfo(Id As String, IsVehical As String, Ids As String) As String
        ...
        Else
            ...
            Info = "<span style='font-weight:bold;'>IMEINO : " + dt.Rows(0).Item("imieno") + "</span> <br>Vehicle Name : " + dt.Rows(0).Item("Site_Name") + " <br>Vehicle No : " + dt.Rows(0).Item("vehicleNo").ToString() + " <br>Speed : " + dt.Rows(0).Item("Speed").ToString() + " Km/h <br>Idle Time : " + dipTime.ToString() + "(HH:MM) <br>Last Record Time : " + dt.Rows(0).Item("ctime").ToString() + " <br> Lattitude : " + dt.Rows(0).Item("Lat").ToString() + "<br>Longitude : " + dt.Rows(0).Item("Long").ToString() + ""
    ...
    Public Shared Function GetSites(str As String) As String

        If HttpContext.Current.Session("USERROLE") = "SU" Then
            Return str
        End If

        Dim Cluster = Convert.ToString(HttpContext.Current.Session("RID"))
        Dim newStr As New StringBuilder()
        Try
            Dim sites = str.Split("|")
            For Each strsite As String In sites
                If strsite = "" Then
                    Continue For
                End If
                If Not Cluster.Contains(strsite.Split("^")(7)) Then
                    Continue For
                End If
                newStr.Append(strsite & "|")
            Next
        Catch ex As Exception
        End Try
