...
Partial Class GPSDataReport2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Dim Vfld As String
            Dim vemei As String
            hdnURI.Value = "http://" + HttpContext.Current.Request.Url.Authority + HttpContext.Current.Request.ApplicationPath & "/GmapWindow.aspx"
    ...
    Protected Sub Show()
        ...
        '        If (ddlrtype.SelectedItem.Text.ToUpper = "NO SIGNAL" And (txtsdate.Text = "" Ortdate = "")) Or (ddlrtype.SelectedItem.Text.ToUpper = "TRIP VS MILEAGE" And (txtsdate.Text = "" Ortdate = "")) Or (ddlrtype.SelectedItem.Text.ToUpper = "NO TRIP" And (txtsdate.Text = "" Ortdate = "")) Then
        ...
        '    Dim edate As String =tdate & " " & txtetime.Text & ":59"
        '    Dim dtt As New DataTable()
        '    If txtsdate.Text = "" Andtdate = "" Then
        ...
        '        ElseIf ddlrtype.SelectedItem.Text.ToUpper = "PENDING TRIP APPROVAL" And ddldhm.SelectedItem.Text.ToUpper = "DAILY" Then
        '            oda.SelectCommand.CommandText = "select U.Username[User Name],E.vehicle_no[Vehicle Number],E.trip_start_datetime[Trip Start DateTime],E.trip_end_datetime[Trip End DateTime],E.trip_start_location[Trip Start Location],E.trip_end_location[Trip End Location],E.Total_Distance[Km. Run],TripType[Trip Type] from mmm_mst_elogbook E left outer join mmm_mst_user u on E.uid=u.uid where E.eid=" & Session("EID") & " and E.imei_no  in  (" & Imieno & ")  and (E.trip_start_datetime>'" & txtsdate.Text & " " & TxtStime.Text & "' and E.trip_start_datetime<'" &tdate & " " & txtetime.Text & "' and E.isauth=0 or E.trip_end_datetime>'" & txtsdate.Text & " " & TxtStime.Text & "' and E.trip_end_datetime<'" &tdate & " " & txtetime.Text & "' and E.isauth=0 and E.trip_end_datetime is not null)"
        '        ElseIf ddlrtype.SelectedItem.Text.ToUpper = "CONSOLIDATED TRIP REPORT" And ddldhm.SelectedItem.Text.ToUpper = "DAILY" Then
        '            oda.SelectCommand.CommandText = "select username[User Name],vehicle_no[Vehicle No.],IMEI_NO[IMEI No.],convert(nvarchar,Trip_Start_DateTime,120)[Trip Start DateTime],convert(nvarchar,Trip_end_DateTime,120)[Trip End DateTime],Trip_Start_Location[Trip Start Location],Trip_End_Location[Trip End Location],Total_Distance[Total Distance],TripType[Trip Type] from MMM_MST_ELOGBOOK e,  MMM_MST_USER u where   e.uid=u.uid and E.imei_no  in  (" & Imieno & ") and E.eid=" & Session("EID") & "  and Trip_start_DateTime >= '" & txtsdate.Text & " " & TxtStime.Text & "'   AND (Trip_end_DateTime <= '" &tdate & " " & txtetime.Text & "' or trip_end_Datetime is null or trip_end_datetime='')   order by Trip_Start_DateTime "
    ...
    Public Shared Function ShowDynamic(ReportType As String, SubType As String, sDate As String, tDate As String, Imieno As String, STime As String, ETime As String, EID As String) As Grid
        Dim objgrid = New Grid()
        Return objgrid
    End Function

    <WebMethod()> _
    Public Shared Function ShowDynamic1(ReportType As String, SubType As String, sDate As String, tDate As String, Imieno As String, STime As String, ETime As String, EID As String) As Grid
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As SqlConnection = New SqlConnection(conStr)
        Dim oda As SqlDataAdapter = New SqlDataAdapter("", con)
        Dim dt As New DataTable
        ...
        oda.Fill(ds, "data")
        Dim objgrid = New Grid()
        If (ds.Tables.Contains("Grid")) Then
        ...
        Try
            ...
            If ReportType.ToUpper <> "LAST SIGNAL REPORT" Or ReportType.ToUpper <> "LAST LOCATION" Then
                If (ReportType.ToUpper = "NO SIGNAL" And (sDate = "" Or tDate = "")) Or (ReportType.ToUpper = "TRIP VS MILEAGE" And (sDate = "" Or tDate = "")) Or (ReportType.ToUpper = "NO TRIP" And (sDate = "" Or tDate = "")) Then
                    objgrid.ErrorMsg = "Please Select date range"
                    Return objgrid

                End If
                If ReportType.ToUpper = "SELECT" Then
                    objgrid.ErrorMsg = "Please Select Report Type"
                    Return objgrid
                End If
                Dim crrdate As String = Date.Now.ToString("yyyy-MM-dd HH:mm")
                If STime = "" Then
                ...
                ElseIf ETime = "" Then
                    ETime = "23:59"
                Else
                    If CDate(sDate) > CDate(tDate) Then
                        objgrid.ErrorMsg = "Date selection is not correct "
                        Return objgrid
                    End If
                    If DateTime.Parse(sDate) > DateTime.Parse(crrdate) Then
                        objgrid.ErrorMsg = "Future start date is not allowed "
                        Return objgrid
                    ElseIf DateTime.Parse(sDate) > DateTime.Parse(crrdate) Then
                        objgrid.ErrorMsg = "Future end date is not allowed "
                        Return objgrid
                    End If
                End If
            End If
            ...
            If sDate = "" And tDate = "" Then
                If ReportType.ToUpper = "IDLE REPORT" And SubType.ToUpper = "DAILY" Then

                ElseIf ReportType.ToUpper = "MILEAGE REPORT" And SubType.ToUpper = "DAILY" Then
                    Query = GetQueryForMileageDaily(EID, Imieno, sDate + " " + STime, tDate + " " + ETime)

                ElseIf ReportType.ToUpper = "PENDING TRIP APPROVAL" And SubType.ToUpper = "DAILY" Then

                    Query = "select U.Username[User Name],E.vehicle_no[Vehicle Number],E.trip_start_datetime[Trip Start DateTime],E.trip_end_datetime[Trip End DateTime],E.trip_start_location[Trip Start Location],E.trip_end_location[Trip End Location],E.Total_Distance[Km. Run],TripType[Trip Type] from mmm_mst_elogbook E left outer join mmm_mst_user u on E.uid=u.uid where E.eid=" & EID & " and E.imei_no  in  (" & Imieno.TrimEnd(",") & ")  and (E.isauth=0 and E.trip_end_datetime is not null)"

                ElseIf ReportType.ToUpper = "LAST SIGNAL REPORT" Then
                    oda.SelectCommand.Parameters.Clear()
                    oda.SelectCommand.CommandType = CommandType.StoredProcedure
                    oda.SelectCommand.CommandText = "usproclastrecord"
                    oda.SelectCommand.Parameters.AddWithValue("imeino", Imieno.ToString.TrimEnd(","))
            ...
            Else
                ...
                ElseIf ReportType.ToUpper = "MILEAGE REPORT" And SubType.ToUpper = "DAILY" Then
                    ...
                    Query = GetQueryForMileageDaily(EID, Imieno, sDate + " " + STime, tDate + " " + ETime)
                ElseIf ReportType.ToUpper = "MILEAGE REPORT" And SubType.ToUpper = "HALF HOURLY" Then
                    Query = "select g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],sum(g.devdist)[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Lat],(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Long]  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and convert(date,ctime)='" & sDate & "' and DATEPART(MINUTE, CAST(cTime as TIME)) IN (0, 30) group by g.IMIENO,g.cTime ORDER BY CAST(ctime as TIME)"
                ElseIf ReportType.ToUpper = "MILEAGE REPORT" And SubType.ToUpper = "ONE HOURLY" Then
                    If Imieno.Split(",").Length > 12 Then
                        objgrid.ErrorMsg = "Please select less than 12 Vehicles."
                        Return objgrid
                        Exit Function
                    End If
                    Query = "select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 00:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 01:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Lat],(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Long]  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 00:00' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 01:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 01:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 02:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Lat],(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Long]  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 01:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 02:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 02:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 03:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Lat],(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Long]  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 02:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 03:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 03:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 04:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 03:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 04:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 04:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 05:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 04:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 05:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 05:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 06:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 05:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 06:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 06:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 07:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 06:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 07:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 07:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 08:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 07:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 08:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 08:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 09:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 08:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 09:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 09:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 10:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 09:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 10:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 10:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 11:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 10:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 11:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 11:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 12:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 11:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 12:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 12:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 13:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 12:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 13:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 13:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 14:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 13:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 14:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 14:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 15:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 14:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 15:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 15:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 16:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 15:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 16:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 16:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 17:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 16:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 17:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 17:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 18:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 17:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 18:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 18:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 19:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 18:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 19:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 19:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 20:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 19:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 20:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 20:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 21:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 20:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 21:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 21:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 22:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 21:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 22:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 22:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 23:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and Dateadd(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 22:01' and Dateadd(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 23:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 23:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 23:59' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,g.ctime),0)>='" & sDate & " 23:01' and DATEADD(minute,DATEDIFF(minute,0,g.ctime),0)<='" & sDate & " 23:59' group by g.IMIENO "
                ElseIf ReportType.ToUpper = "MILEAGE REPORT" And SubType.ToUpper = "TWO HOURLY" Then
                    If Imieno.Split(",").Length > 12 Then
                        objgrid.ErrorMsg = "Please select less than 12 Vehicles."
                        Return objgrid
                        Exit Function
                    End If
                    Query = "select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 00:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 02:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Lat],(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))[Long]  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 00:01' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 02:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 02:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 04:00' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 02:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 04:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 04:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 06:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 04:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 06:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 06:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 08:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 06:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 08:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 08:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 10:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 08:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 10:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 10:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 12:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 10:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 12:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 12:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 14:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 12:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 14:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 14:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 16:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 14:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 16:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 16:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 18:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 16:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 18:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 18:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 20:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 18:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 20:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,ctime),0)>='" & sDate & " 20:00' and DATEADD(minute,DATEDIFF(minute,0,ctime),0)<='" & sDate & " 22:00' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)>='" & sDate & " 20:00' and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)<='" & sDate & " 22:00' group by g.IMIENO union select distinct  g.IMIENO[IMEINO], max(m.FLD10)[VehicleNo],max(m1.fld1)[Circle],(select isnull(sum(devdist),0) from mmm_mst_gpsdata where imieno=g.imieno and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)>='" & sDate & " 22:00' and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)<='" & sDate & " 23:59' and ((devdist>0.06 and speed=0) or (devdist>0 and speed>0)))[TotalKM],max(g.ctime)[Date],(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName],(select top 1 lattitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime)),(select top 1 longitude from mmm_mst_gpsdata where imieno=g.imieno and ctime=max(g.ctime))  from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2 inner join mmm_mst_master m1 on m1.tid=m.fld18  where m.documenttype='VEHICLE' and m1.documenttype='Circle office' and m.eid=32  and g.imieno in (" + Imieno.TrimEnd(",") + ") and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)>='" & sDate & " 22:00' and DATEADD(minute,DATEDIFF(minute,0,GETDATE()),0)<='" & sDate & " 23:59' group by g.IMIENO"
                ElseIf ReportType.ToUpper = "TRIP VS MILEAGE" Then
                    'New Change
                    Query = "select r1.VehicleNo,r1.IMEINO,r1.Circle,r1.TotalKM,r1.date1[date],r2.distance[Trip Distance KM],r2.start[Trip start],r2.Trip_end_datetime[Trip end datetime],r1.UserName from (select distinct g.IMIENO[IMEINO], m.FLD10[VehicleNo],(select fld1 from mmm_mst_master where tid=m.fld18)[Circle],sum(devdist)[TotalKM],convert(nvarchar,g.ctime,5)[Date1] ,(select * from  GETUSERNAMEBYIMEINO(g.IMIENO))[UserName] from mmm_mst_master m inner join mmm_mst_gpsdata g on g.imieno=m.fld2  where m.documenttype='VEHICLE' and m.eid=" & EID & "  and g.imieno in (" + Imieno.TrimEnd(",") + ") and g.cTime >= '" & sDate & "' AND g.ctime <= '" & tDate & "' and ((devdist>0.03 and speed=0) or (devdist>0 and speed>0))  group by g.imieno,m.fld10,m.fld18,convert(nvarchar,g.ctime,5),m.fld2 ) r1 left outer join  ( select imei_no[imeino1],Total_distance[distance],Trip_start_datetime[start],Trip_end_datetime from MMM_mst_elogbook where imei_no in (" + Imieno.TrimEnd(",") + ") and Trip_start_datetime >='" & sDate & "' and  '" & tDate & "'>=Trip_start_datetime and  eid=" & EID & ") as r2 on r1.IMEINO=r2.imeino1 and convert(date,r2.start,120)= convert(date,r1.date1,5) order by [UserName],[date]"
                ElseIf ReportType.ToUpper = "NO SIGNAL" And SubType.ToUpper = "DAILY" Then
                    oda.SelectCommand.Parameters.Clear()
                    oda.SelectCommand.CommandType = CommandType.StoredProcedure
                    oda.SelectCommand.CommandText = "NOSIGNALBYI"
                    oda.SelectCommand.Parameters.AddWithValue("imieno", Imieno.TrimEnd(",").ToString)
                    oda.SelectCommand.Parameters.AddWithValue("sdatetime", sDate)
                    oda.SelectCommand.Parameters.AddWithValue("edatetime", tDate)
                    Try
                        oda.SelectCommand.CommandTimeout = 300
                        oda.Fill(ds, "Grid")
                    Catch exption As Exception
                        objgrid.ErrorMsg = "Dear User please enter short date range "
                        Return objgrid
                    End Try
                ElseIf ReportType.ToUpper = "PENDING TRIP APPROVAL" And SubType.ToUpper = "DAILY" Then
                    Query = "select U.Username[User Name],E.vehicle_no[Vehicle Number],E.trip_start_datetime[Trip Start DateTime],E.trip_end_datetime[Trip End DateTime],E.trip_start_location[Trip Start Location],E.trip_end_location[Trip End Location],E.Total_Distance[Km. Run],TripType[Trip Type] from mmm_mst_elogbook E left outer join mmm_mst_user u on E.uid=u.uid where E.eid=" & EID & " and E.imei_no  in  (" & Imieno & ")  and (E.trip_start_datetime>'" & sDate & " " & STime & "' and E.trip_start_datetime<'" & tDate & " " & ETime & "' and E.isauth=0 or E.trip_end_datetime>'" & sDate & " " & STime & "' and E.trip_end_datetime<'" & tDate & " " & ETime & "' and E.isauth=0 and E.trip_end_datetime is not null)"
                ElseIf ReportType.ToUpper = "CONSOLIDATED TRIP REPORT" And SubType.ToUpper = "DAILY" Then
                    Query = "select username[User Name],vehicle_no[Vehicle No.],IMEI_NO[IMEI No.],convert(nvarchar,Trip_Start_DateTime,120)[Trip Start DateTime],convert(nvarchar,Trip_end_DateTime,120)[Trip End DateTime],Trip_Start_Location[Trip Start Location],Trip_End_Location[Trip End Location],Total_Distance[Total Distance],TripType[Trip Type] from MMM_MST_ELOGBOOK e,  MMM_MST_USER u where   e.uid=u.uid and E.imei_no  in  (" & Imieno & ") and E.eid=" & EID & "  and Trip_start_DateTime >= '" & sDate & " " & STime & "'   AND (Trip_end_DateTime <= '" & tDate & " " & ETime & "' or trip_end_Datetime is null or trip_end_datetime='')   order by Trip_Start_DateTime "
                ElseIf ReportType.ToUpper = "NO TRIP" And SubType.ToUpper = "DAILY" Then
                    If Imieno.ToString.Length > 17 Then
                        objgrid.ErrorMsg = "Please select only one User-Vehicle at a time"
                        Return objgrid
                        Exit Function
                    Else
                        objgrid.ErrorMsg = ""
            ...
            Using con
                If Not (ds.Tables.Contains("Grid")) Then
            ...
            Try
                For j As Integer = 0 To ds.Tables("Grid").Columns.Count - 1
                    Dim objgrodcol As New GridCol()
                    Dim colnm = ds.Tables("Grid").Columns(j).ColumnName.Replace(" ", "_").Replace(".", "").ToString().Replace("&", "and").ToString()
                    objgrodcol.field = colnm
                    objgrodcol.title = ds.Tables("Grid").Columns(j).ColumnName
                    Columns.Add(objgrodcol)
                    ds.Tables("Grid").Columns(j).ColumnName = colnm
                Next

                Dim serializerSettings As New JsonSerializerSettings()
                Dim json_serializer As New JavaScriptSerializer()
                serializerSettings.Converters.Add(New DataTableConverter())
                Dim jsonData As String = JsonConvert.SerializeObject(ds.Tables("Grid"), Newtonsoft.Json.Formatting.None, serializerSettings)
    ...
    Shared Function GetQueryForMileageDaily(Eid As String, IMEINos As String, d1 As String, d2 As String) As String
        Dim query As String = " Select * "
        Dim Groupby As String = " group by "
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As SqlConnection = New SqlConnection(conStr)
        Dim dt As New DataTable()
        Dim oda As SqlDataAdapter = New SqlDataAdapter("select * from mmm_mst_gpsreportsettings where Reporttype ='MileageReport' and eid =" + Eid + " order by SequenceNo", con)
        oda.Fill(dt)
        ...
        If (dtGrid.Length > 0) Then
            ...
            For i As Integer = 0 To dtGrid.Length - 1
                ...
                If (dtGrid(i).Item("VType") = "g.cTime") Then

                    query &= "Max( " + dtGrid(i).Item("DocField").ToString().Split(":")(0) + ")[" + dtGrid(i).Item("DispName").ToString() + "],"
                ...
                If Not (dtGrid(i).Item("DocField").ToString().Contains("DevDist") Or dtGrid(i).Item("DocField").ToString().Contains("speed") Or dtGrid(i).Item("DocField").ToString().Contains("DevDist")) Then
                    If (dtGrid(i).Item("DocField").ToString().Contains("g.cTime")) Then
                        Groupby &= "Convert(date," + dtGrid(i).Item("VType").ToString() + "),"
                    Else
                        Groupby &= dtGrid(i).Item("VType").ToString() + ","
                    End If
        ...
        If (dtVehicle.Length > 0) Then
            ...
            query &= ", max(g.tid)[TID],'Automatic' [TripType] from (select * from  mmm_mst_gpsdata where IMIENO in (" & IMEINos.TrimEnd(",") & ") and convert(dateTime,ctime) " &
"between convert(datetime,'" & d1 & "') and convert(datetime,'" & d2 & "')) g " &
" inner join (select * from  mmm_mst_master where eid =" & Eid & " and Documenttype ='" & dtVehicle(0).Item("VType").ToString() & "' ) Vehicle " &
"  on g.IMIENO=Vehicle." & dtVehicle(0).Item("DocField").ToString()

        Else

            query &= ", max(g.tid)[TID],'Automatic' [TripType] from (select * from  mmm_mst_gpsdata where IMIENO in (" & IMEINos.TrimEnd(",") & ") and convert(datetime,ctime) between convert(datetime,'" & d1 & "') and convert(datetime,'" & d2 & "')) g " &
" inner join (select * from  mmm_mst_master where eid =" & Eid & " and Documenttype ='Vehicle' ) Vehicle " &
"  on g.IMIENO=Vehicle." & dtVehicle(0).Item("DocField").ToString()

        End If

        Return query + Groupby.TrimEnd(",")
    ...
    Protected Sub btnshow_Click1(sender As Object, e As ImageClickEventArgs) Handles btnshow.Click
