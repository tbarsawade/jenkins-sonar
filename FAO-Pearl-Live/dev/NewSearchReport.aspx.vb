...
Partial Class NewSearchReport
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Try
                ...
                '' 
                da.SelectCommand.CommandText = "select ReportName,ReportDescription from mmm_mst_report where eid=" & Session("EID") & " and reportid=1618 "
                da.Fill(ds, "data")
    ...
    Protected Shared Function GenearateQuery(data As Dictionary(Of String, Object), IsActionForm As Boolean) As DGrid
        ...
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As New SqlConnection(conStr)
        Dim da As New SqlDataAdapter("select qryfield from mmm_mst_report where eid=" & eid & "  and Reportid=1618 ", con)
        If con.State <> ConnectionState.Open Then
            con.Open()
        End If
        ...
        If HttpContext.Current.Session("DDlField") = "adate" Then
            If data("frdate").ToString() <> "" Then
                str = Replace(str, "@filter1", "adate>='" & data("frdate").ToString() & "' and adate<='" & data("todate").ToString() & "'")
                str = Replace(str, "@filter2", "adate>='" & data("frdate").ToString() & "' and adate<='" & data("todate").ToString() & "'")
                str = Replace(str, "@filter3", "adate>='" & data("frdate").ToString() & "' and adate<='" & data("todate").ToString() & "'")
            End If
        End If
    ...
    Public Function GenearateQueryExl(EID As Integer) As DataTable
        ...
        If HttpContext.Current.Session("DDlField") = "adate" Then
            If txtsdate.Text.ToString() <> "" Then
                str = Replace(str, "@filter1", "adate>='" & txtsdate.Text.ToString() & "' and adate<='" & txtedate.Text.ToString() & "'")
                str = Replace(str, "@filter2", "adate>='" & txtsdate.Text.ToString() & "' and adate<='" & txtedate.Text.ToString() & "'")
                str = Replace(str, "@filter3", "adate>='" & txtsdate.Text.ToString() & "' and adate<='" & txtedate.Text.ToString() & "'")
            End If
        End If
        If HttpContext.Current.Session("DDlField") = "curstatus" Then
        ...
        'If HttpContext.Current.Session("DDlField") = "tid" Then
        '    If txtval.Text.ToString() <> "" Then
        '        str = Replace(str, "@filter1", " tid like '%" & txtval.Text.ToString() & "%'")
        '        str = Replace(str, "@filter2", " tid like '%" & txtval.Text.ToString() & "%'")
        '        str = Replace(str, "@filter3", " tid like '%" & txtval.Text.ToString() & "%'")
        '    End If
        'End If
        If HttpContext.Current.Session("DDlField") = "Invoice Number" Then
            If txtval.Text.ToString() <> "" Then
