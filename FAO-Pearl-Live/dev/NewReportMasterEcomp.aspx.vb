...
Partial Class NewReportMasterEcomp
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Else
            ...
            Try
                ...
                If (scrname = "Act Report") Then
                    oda.SelectCommand.CommandText &= " ,displayname"
                End If
                ...
                ' dv.Sort = " fieldmapping Desc "


                'ob.CreateControlsOnAdvanceSearch(ds.Tables("data"), pnlFields)
                'ob.CreateControlsOnReport(ds.Tables("data"), pnlFields) 
                Dim dtfinfields As New DataTable()
                If (scrname = "Act Report") Then
                    Dim dtfld115 As New DataTable
                    oda.SelectCommand.CommandText = "SELECT *  FROM MMM_MST_FIELDS  where Fieldid =25587 "
                    oda.Fill(dtfld115)
                    Dim Row As DataRow = dtfld115.Rows(0)
                    '  ds.Tables("data").Rows.InsertAt(Row, 0)
                    dtfinfields.Merge(dtfld115)
                    dtfinfields.Merge(ds.Tables("data"))

                    '   ds.Tables("data").Merge(dtfld115)
                    '      ds.Tables("data").Merge(
                End If

                CreateControlsOnReport(dtfinfields, pnlFields, scrname)
    ...
    Public Sub CreateControlsOnReport(ByVal ds As DataTable, ByRef pnlFields As Panel, Optional ReportName As String = "")
        ...
        Try
            ...
            For i As Integer = 0 To ds.Rows.Count - 1
                ...
                'pnlFields.Controls.Add(New LiteralControl("<tr>"))
                'End If
                'Changed by Ajeet Kumar
                pnlFields.Controls.Add(New LiteralControl("<td class='td_style'>"))
                pnlFields.Controls.Add(lbl)
                pnlFields.Controls.Add(New LiteralControl("</td><td class='td_style1'>"))
                ...
                Select Case ds.Rows(i).Item("FieldType").ToString().ToUpper()
                    ...
                    Case "DROP DOWN"
                        ...
                        chkAll.ID = "chkAll" & ds.Rows(i).Item("FieldID").ToString()
                        chkAll.CssClass = "Mytxtbox"
                        chkAll.Text = "Select All"
                        chkAll.InputAttributes("onchange") = "r('" + chkAll.ClientID + "','" + "chklist" & ds.Rows(i).Item("FieldID").ToString() + "')"
                        ...
                        'If (ReportName = "Act Report") Then
                        '    Dim chkbox As New CheckBox()
                        '    chkbox.ID = "chkselectall_" & ds.Rows(i).Item("FieldID").ToString()
                        '    chkbox.Text = "Select All"
                        '    dynmdiv.Controls.Add(chkbox)
                        'End If
                        dynmdiv.Controls.Add(chkAll)
                        Dim chklist As New CheckBoxList
                        chklist.ID = "chklist" & ds.Rows(i).Item("FieldID").ToString()
                        chklist.CssClass = "lblcss"
                        ...
                        ElseIf UCase(dropdowntype) = "MASTER VALUED" Then
                            ...
                            If arr(1) = "Site Contractor Master" Then
                                If (Session("USERROLE") = "SU" Or Session("USERROLE") = "ADMIN") Then
                                    str = "select fld1 ,tid from " & TABLENAME & " WHERE EID=" & HttpContext.Current.Session("EID") & " AND  DOCUMENTTYPE='Contractor Master'"
                                Else
                                    str = "if EXISTS (select docmapping from mmm_mst_forms where eid =98 and  formname ='Contractor Master' and isroledef=1) " &
                                    " begin	 declare @docmappingcont as varchar(200) select @docmappingcont = docmapping from mmm_mst_forms where eid =98 and  formname ='Contractor Master' and isroledef=1  " &
 " declare @Qrystrcontractor as varchar(2000)  set @Qrystrcontractor = 'select fld1,tid from mmm_mst_master where eid =98 and documenttype =''Contractor Master''  and tid in (select items from dbo.split((select '+ @docmappingcont +' " &
  " from mmm_ref_role_user where eid =98 and uid =" & Session("UID") & " and rolename =''" & Session("USERROLE") & "''),'','')  by fld100 '  exec (@Qrystrcontractor) End  else  select fld1 ,tid from mmm_mst_master  where eid =98 and documenttype ='Contractor master'   order by fld1 "

                                End If
                            ElseIf arr(1) = "Acts Applicable To Site" Then
                                str = "select fld1 ,tid from " & TABLENAME & " WHERE EID=" & HttpContext.Current.Session("EID") & " AND  DOCUMENTTYPE='Act Master'"
                            ElseIf arr(1) = "Company Master" Then
                                If (Session("USERROLE") = "SU" Or Session("USERROLE") = "ADMIN") Then
                                    str = "select fld1 ,tid from " & TABLENAME & " WHERE EID=" & HttpContext.Current.Session("EID") & " AND  DOCUMENTTYPE='Company Master'"
                                Else
                                    str = "select fld1 ,tid from " & TABLENAME & " WHERE EID=" & HttpContext.Current.Session("EID") & " AND  DOCUMENTTYPE='Company Master'" &
                                        " and tid in  (select items from dbo.split((select fld1 from mmm_ref_role_user where eid =98 and uid =" & Session("UID") & "),',')  )"

                                End If
                            ElseIf arr(1) = "Site Master" Then
                                If (Session("USERROLE") = "SU" Or Session("USERROLE") = "ADMIN") Then
                                    str = " select fld100 ,tid from mmm_mst_master  where eid =98 and documenttype ='Site master'   order by fld100"
                                Else
                                    str = "if EXISTS (select docmapping from mmm_mst_forms where eid =98 and  formname ='Site Master' and isroledef=1) " &
...
" and tid in (select items from dbo.split((select '+ @docmapping +' from mmm_ref_role_user where eid =98 and uid =" & Session("UID") & " and rolename =''" & Session("USERROLE") & "''),'','')  ) order by fld100 ' exec (@Qrystr) End else  " &
" select fld100 ,tid from mmm_mst_master  where eid =98 and documenttype ='Site master'   order by fld100"
            ...
            If ds.Rows.Count > 0 Then
                ...
                ElseIf (ds.Rows(1).Item("documenttype").ToString.ToUpper = "ACT DOCUMENT") Then
                    Dim lbldocdate As New Label
                    lbldocdate.ID = "Frlbl01"
                    ...
                    pnlFields.Controls.Add(New LiteralControl("<td style=""width:" & lblWidth & "px;text-align:right; vertical-align: top;"">"))
                    pnlFields.Controls.Add(lbldocdate)
                    pnlFields.Controls.Add(New LiteralControl("</td><td style=""width:" & controlWdth & "px;text-align:left; vertical-align: top;"" nowrap='nowrap'>"))
                    Dim txtBox As New TextBox
                    Dim txtBox1 As New TextBox
                    Dim lblt As New Label
                    Dim lblf As New Label
                    ...
                    pnlFields.Controls.Add(New LiteralControl("</td><td style=""width:" & controlWdth & "px;text-align:left; vertical-align: top;"" nowrap='nowrap'>"))
                    pnlFields.Controls.Add(New LiteralControl("To<br/>"))
                    pnlFields.Controls.Add(txtBox1)
                    Dim CLNDR As New AjaxControlToolkit.CalendarExtender
                    Dim CLNDR1 As New AjaxControlToolkit.CalendarExtender
                ...
                ElseIf Not (ds.Rows(0).Item("documenttype").ToString.ToUpper = "ACT DOCUMENT") Then
                    Dim lbldocdate As New Label
                    lbldocdate.ID = "Frlbl01"
                    lbldocdate.Text = "Document Date"
                    lbldocdate.Font.Bold = True
                    ...
                    pnlFields.Controls.Add(New LiteralControl("<td class='td_Style' >"))
                    pnlFields.Controls.Add(lbldocdate)
                    pnlFields.Controls.Add(New LiteralControl("</td><td class='td_Style'>"))
                    Dim txtBox As New TextBox
                    Dim txtBox1 As New TextBox
                    Dim lblt As New Label
                    Dim lblf As New Label
    ...
    Protected Shared Function showViewOffReport1(data As Dictionary(Of String, Object)) As DGrid
        ...
        Try
            ...
            da.SelectCommand.CommandTimeout = 600
            ReportQuery = "select d.tid as DocID, dms.udf_split('MASTER-Acts Applicable To Site-fld4', d.fld2) Acts, (case when d.fld25 ='PE' then d.fld47  when d.fld25 ='Contractor' then d.fld46 end )[Activity] ,d.fld44 [Contractor_Name], dms.udf_split('MASTER-Site Master-fld100', d.fld7) Site, dms.udf_split('MASTER-Company Master-fld1',d.fld5) [Company],dms.udf_split('MASTER-State-fld1',Site.fld101) State,dms.udf_split('MASTER-Customer Master-fld1', Site.fld115) Customer, (CASE WHEN convert(date,d.fld4,3) <= dateadd(dd,10,convert(date,d.fld43,3)) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) <= dateadd(dd,10,convert(date,d.fld43,3)) THEN 'Performed' WHEN (convert(date,d.fld4,3) <= convert(date,d.fld43,3)) AND (convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) > dateadd(dd,10,convert(date,d.fld43,3))) THEN 'Delayed' WHEN (convert(date,d.fld4,3) > convert(date,d.fld43,3) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) <= dateadd(dd,10,convert(date,d.fld43,3)) ) OR ( convert(date,d.fld4,3) > convert(date,d.fld43,3) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) > dateadd(dd,10,convert(date,d.fld43,3))) OR (d.CurStatus ='Pending Action From Maker' AND convert(date,getdate(),3) > dateadd(dd,10,convert(date,d.fld43,3))) OR (d.CurStatus ='Pending For Verifier' AND convert(date,getdate(),3) > dateadd(dd,10,convert(date,d.fld43,3)))  THEN 'Performed After Due Date' WHEN (d.CurStatus ='Pending Action From Maker' AND convert(date,getdate(),3) < dateadd(dd,10,convert(date,d.fld43,3))) OR (d.CurStatus ='Pending For Verifier' AND convert(date,getdate(),3) < dateadd(dd,10,convert(date,d.fld43,3))) THEN 'In Process' ELSE '' END )  Action from mmm_mst_doc d inner join mmm_mst_master site  on d.fld7 = site.tid where d.eid =98 and d.documenttype ='act document' and site.eid =98 and site.documenttype ='Site Master' and d.fld5 in (@fld5) and (isnull(d.fld1,0) in ( select tid from mmm_mst_master where eid =98 and documenttype ='Site Contractor Master' and fld4 in ( @fld1)) ) and d.fld7 in (@fld7) and d.fld2 in ( select tid from mmm_mst_master where eid =98 and documenttype ='Acts Applicable To Site' and fld3 in (@fld2)) and convert(date,d.adate ) > = convert(date,@adate) and convert(date, d.adate) <= convert(date,$adate)"

            '(CASE WHEN (d.fld4 is null AND convert(date,getdate(),3) > dateadd(dd,10,convert(date,d.fld43,3))) OR (convert(date,d.fld4,3) > convert(date,d.fld43,3) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) > dateadd(dd,10,convert(date,d.fld43,3)) ) OR ( convert(date,d.fld4,3) <= convert(date,d.fld43,3) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) > dateadd(dd,10,convert(date,d.fld43,3)) ) OR ( convert(date,d.fld4,3) > convert(date,d.fld43,3)) THEN 'Performed After Due Date' WHEN convert(date,d.fld4,3) <= dateadd(dd,10,convert(date,d.fld43,3)) AND convert(date,(select tdate from MMM_DOC_DTL WHERE docID =d.tid and aprstatus ='Pending For Verifier'),3) <= dateadd(dd,10,convert(date,d.fld43,3)) THEN 'Performed' ELSE 'Delayed' END) Action

            'Changes vinod 11 Aug 2016
            'da.SelectCommand.ExecuteScalar().ToString()
            Dim sb As New StringBuilder()

            If (data.ContainsKey("fld115_25587")) Then
                If ((data.Values(0)).length > 0) Then
                    ReportQuery &= "and site.fld115 in (@fld115) "
                End If
            End If
            ...
            If (scrname = "Act Report") Then
                ...
                Dim dtfld115 As New DataTable
                da.SelectCommand.CommandText = "SELECT *  FROM MMM_MST_FIELDS  where Fieldid =25587 "
                da.Fill(dtfld115)
                ds.Tables("flds").Merge(dtfld115)

            End If

            Dim dv As New DataView(ds.Tables("flds"))
            dv.Sort = "FieldMapping desc"
            Dim dt = dv.ToTable()
            'ds.Tables("flds").Clear()
            ...
            For Each row As DataRow In dt.Rows
                ...
                ElseIf fldType = "Drop Down" Then
                    ...
                    Else
                        ...
                        If (fldmapping = "fld1" And scrname = "Act Report") Then
                            ReportQuery = Replace(ReportQuery, "select tid from mmm_mst_master where eid =98 and documenttype ='Site Contractor Master' and fld4 in ( @fld1)", "isnull(d." & fldmapping & ",0) ")
                        ElseIf (fldmapping = "fld2" And scrname = "Act Report") Then
                            ReportQuery = Replace(ReportQuery, "select tid from mmm_mst_master where eid =98 and documenttype ='Acts Applicable To Site' and fld3 in (@fld2)", "d." & fldmapping)
                        Else
            ...
            Else
                ...
                Else
                    If data(key1).ToString.Trim <> "" Then
                        ReportQuery = Replace(ReportQuery, "@adate", "Convert(date,'" & data(key1).ToString & "',3)")
                    Else
                        ReportQuery = Replace(ReportQuery, "@adate", "convert(date,d.adate)")
                    End If
                    If data(key2).ToString.Trim <> "" Then
                        ReportQuery = Replace(ReportQuery, "$adate", "Convert(date,'" & data(key2).ToString.Trim & "',3)")
                    Else
                        ReportQuery = Replace(ReportQuery, "$adate", "convert(date,d.adate)")
                    End If
                End If
            ...
            Dim dtsort As DataTable = ds.Tables("ss")
            Dim count = dtsort.Select("Contractor_Name <> ''").Count()

            If (count = 0) Then

                ds.Tables("ss").Columns.Remove("Contractor_Name")
            End If
            ...
            grid.Chart = GetChartDataEcompbeta(ds.Tables("ss"))
    ...
    Public Shared Function GetChartDataEcompbeta(dt As DataTable) As String
        ...
        Try
            ...
            For i As Integer = 0 To dtCustomer.Rows.Count - 1
                ...
                Dim per As Integer = 0
                Dim Notper As Integer = 0
                Dim PerAftrDue As Integer = 0
                For j As Integer = 0 To dt.Rows.Count - 1
                    If dt.Rows(j).Item("Customer").ToString().ToUpper() = customer Then
                        If dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED" Then
                            per = per + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED AFTER DUE DATE" Then 'NOT PERFORMED
                            Notper = Notper + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "DELAYED" Then 'PERFORMED AFTER DUE DATE
                            PerAftrDue = PerAftrDue + 1
                        End If
                    End If
                Next
                strData.Append(customer & "," & per & "," & Notper & "," & PerAftrDue & "|")
            Next

            strData.Append("==")

            Dim dtSites = dv.ToTable(True, "Site")
            For i As Integer = 0 To dtSites.Rows.Count - 1
                Dim site As String = Convert.ToString(dtSites.Rows(i).Item(0)).ToUpper()
                Dim per As Integer = 0
                Dim Notper As Integer = 0
                Dim PerAftrDue As Integer = 0
                For j As Integer = 0 To dt.Rows.Count - 1
                    If dt.Rows(j).Item("Site").ToString().ToUpper() = site Then
                        If dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED" Then
                            per = per + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED AFTER DUE DATE" Then
                            Notper = Notper + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "DELAYED" Then
                            PerAftrDue = PerAftrDue + 1
                        End If
                    End If
                Next
                strData.Append(site & "," & per & "," & Notper & "," & PerAftrDue & "|")
            Next
            strData.Append("==")

            Dim dtAct = dv.ToTable(True, "Acts")
            For i As Integer = 0 To dtAct.Rows.Count - 1
                Dim Act As String = Convert.ToString(dtAct.Rows(i).Item(0)).ToUpper()
                Dim per As Integer = 0
                Dim Notper As Integer = 0
                Dim PerAftrDue As Integer = 0
                For j As Integer = 0 To dt.Rows.Count - 1
                    If dt.Rows(j).Item("Acts").ToString().ToUpper() = Act Then
                        If dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED" Then
                            per = per + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED AFTER DUE DATE" Then
                            Notper = Notper + 1
                    ...
                    End If
                Next
                strData.Append(Act & "," & per & "," & Notper & "," & PerAftrDue & "|")
            Next

            strData.Append("==")
            If (dt.Columns.Contains("Contractor_Name")) Then
                ...
                For i As Integer = 0 To dtContractor.Rows.Count - 1
                    ...
                    If (Contractor <> "") Then
                        For j As Integer = 0 To dt.Rows.Count - 1
                            ...
                            End If
                        Next
                        strData.Append(Contractor & "," & per & "," & Notper & "," & PerAftrDue & "|")
                    End If
            ...
            Dim dtCompany = dv.ToTable(True, "Company")
            For i As Integer = 0 To dtAct.Rows.Count - 1
                Dim Company As String = Convert.ToString(dtCompany.Rows(i).Item(0)).ToUpper()
                Dim per As Integer = 0
                Dim Notper As Integer = 0
                Dim PerAftrDue As Integer = 0
                For j As Integer = 0 To dt.Rows.Count - 1
                    If dt.Rows(j).Item("Company").ToString().ToUpper() = Company Then
                        If dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED" Then
                            per = per + 1
                        ElseIf dt.Rows(j).Item("Action").ToString().ToUpper() = "PERFORMED AFTER DUE DATE" Then
                            Notper = Notper + 1
                    ...
                    End If
                Next
                strData.Append(Company & "," & per & "," & Notper & "," & PerAftrDue & "|")
            Next
