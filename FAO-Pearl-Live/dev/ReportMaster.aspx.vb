...
Partial Class ReportMaster
    ...
    End Sub
    Public Function fldmapping() As String
        Dim scrname As String = Request.QueryString("SC").ToString()
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        ...
        Try
            ...
            If Session("USERROLE") = "SU" Then
                abc.SelectCommand.CommandText = "select qryfield from mmm_mst_report where subentity ='" & xy & "' and eid=" & Session("EID") & " and reportname= '" & scrname & "'"
            Else
                abc.SelectCommand.CommandText = "select qryfieldrole from mmm_mst_report where subentity ='" & xy & "' and eid=" & Session("EID") & " and reportname= '" & scrname & "'"
            End If
    ...
    Public Function Viewfldmapping() As String
        Dim scrname As String = Request.QueryString("SC").ToString()
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As SqlConnection = New SqlConnection(conStr)
        Dim abc As SqlDataAdapter = New SqlDataAdapter("select subentity from MMM_MST_REPORT where eid=" & Session("eid") & " and reportname= '" & scrname & "'", con)
    ...
    Protected Sub showViewOffReport()
        ...
        Try
            ...
            Else
                ...
                Else
                    If txtobjdate.Text <> "" Then
                        ss = Replace(ss, "@adate", "Convert(date,'" & txtobjdate.Text & "',3)")
                    Else
                        ss = Replace(ss, "@adate", "convert(date,d.adate)")
                    End If
                    If txtobjdate1.Text <> "" Then
                        ss = Replace(ss, "$adate", "Convert(date,'" & txtobjdate1.Text & "',3)")
                    Else
                        ss = Replace(ss, "$adate", "convert(date,d.adate)")
                    End If
                End If
            ...
            'For JV REPORT 
            If scrname.ToUpper = "JV REPORT" Or scrname.ToUpper = "APPROVED PETTY CASH VOUCHER HUB REPORT" Or scrname.ToUpper = "REJECTED PETTY CASH VOUCHER HUB REPORT" Or scrname.ToUpper = "APPROVED UTILITY VOUCHER REPORT" Or scrname.ToUpper = "REJECTED UTILITY VOUCHER REPORT" Then
                Dim txtobjdate As New System.Web.UI.WebControls.TextBox
                Dim txtobjdate1 As New System.Web.UI.WebControls.TextBox
                txtobjdate = TryCast(pnlFields.FindControl("Frfldtxtdate"), System.Web.UI.WebControls.TextBox)
                ...
                Else
                    ...
                    ss = Replace(ss, "$adate", "convert(date,d.adate)")
                End If
            ElseIf scrname.ToUpper = "CASH MASTER REPORT" Then
                Dim txtobjdate As New System.Web.UI.WebControls.TextBox
                Dim txtobjdate1 As New System.Web.UI.WebControls.TextBox
                txtobjdate = TryCast(pnlFields.FindControl("Frfldtxtdate"), System.Web.UI.WebControls.TextBox)
                txtobjdate1 = TryCast(pnlFields.FindControl("FrfldtxtTodate"), System.Web.UI.WebControls.TextBox)
                If txtobjdate.Text <> "" And txtobjdate1.Text <> "" Then
                    ss = Replace(ss, "@Frtdate", "Convert(date,'" & txtobjdate.Text & "')")
                    ss = Replace(ss, "@Todate", "Convert(date,'" & txtobjdate1.Text & "')")
                    ss = Replace(ss, "@adate", "convert(date,d.adate)")
                    ss = Replace(ss, "$adate", "convert(date,d.adate)")
                Else
                    ss = Replace(ss, "@Frtdate", "Convert(date,d.adate)")
                    ss = Replace(ss, "@Totdate", "Convert(date,d.adate)")
                    ss = Replace(ss, "@adate", "convert(date,d.adate)")
                    ss = Replace(ss, "$adate", "convert(date,d.adate)")
                End If
            ...
            da.SelectCommand.CommandText = "select count(*) from MMM_PreRole_dataFilter where eid= " & Session("EID") & " and rolename='" & Session("USERROLE") & "'"
            Dim apre As Integer = da.SelectCommand.ExecuteScalar()
            If apre <> "0" Then
                ss = Replace(ss, "@uid", Session("UID"))
                ss = Replace(ss, "@role", "'" & Session("USERROLE") & "'")
            End If
            ...
            Mydatatable = ds.Tables("ss")
            If ds.Tables("ss").Rows.Count > 0 Then
                If scrname.ToUpper = "FBH REPORT" Then
                    Dim cdt As DataTable = crosstab(ds.Tables("ss"))
                    gvReport.DataSource = cdt
                    ViewState("xlexport") = cdt
                Else
                    gvReport.DataSource = ds.Tables("ss")
                End If
                gvReport.DataBind()
    ...
    Protected Sub gvReport_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvReport.RowDataBound
        Try
            Dim scrname As String = Request.QueryString("SC").ToString()
            If scrname.ToString = "SKU Wise Norm Report" Then
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    End If
                    If quantity > 1 And quantity <= 33 Then
                        cell.BackColor = Color.Red
                    End If
                    If quantity > 34 AndAlso quantity <= 66 Then
                        cell.BackColor = Color.Yellow
                    End If
                    If quantity > 67 AndAlso quantity <= 100 Then
    ...
    Protected Sub btnViewInExcel_Click(sender As Object, e As EventArgs) Handles btnViewInExcel.Click
        Try
            ...
            Else
                ...
                show()
                If lblCaption.Text = "No Records Found.." Then
                    Exit Sub
                End If
                gvReport.AllowPaging = False
                gvReport.AllowSorting = False
    ...
    End Sub
    Function crosstab(ByVal dt As DataTable) As DataTable
        Dim tbl As New DataTable
        Dim rttbl As New DataTable
        Dim rttbl2 As New DataTable
        Dim dv As New DataView()
        dv = dt.DefaultView
        rttbl = dv.ToTable(True, "Acts")
        Dim dv1 As New DataView()
        dv1 = dt.DefaultView
        rttbl2 = dv1.ToTable(True, "Sites")

        For i As Integer = 0 To rttbl2.Rows.Count - 1
            rttbl.Columns.Add(Convert.ToString(rttbl2.Rows(i)(0)))
        Next

        rttbl.Columns.Add("Total Comp")
        rttbl.Columns.Add("Complied status")
        rttbl.Columns.Add("% Complied")

        For j = 1 To rttbl.Columns.Count - 1
            For k = 0 To dt.Rows.Count - 1
                If Convert.ToString(dt.Rows(k)(0)) = Convert.ToString(rttbl.Rows(j)(0)) And Convert.ToString(dt.Rows(k)(1)) = Convert.ToString(rttbl.Columns(j).ToString()) Then
                    For i As Integer = 0 To rttbl.Rows.Count - 1
                        If IsDBNull(dt.Rows(i)(2)) = False Then
                            rttbl.Rows(i)(rttbl.Columns(j)) = dt.Rows(k).Item(2).ToString()

                            rttbl.Rows(i)("Total Comp") = dt.Rows(k).Item("Total Comp").ToString()
                            rttbl.Rows(i)("Complied status") = dt.Rows(k).Item("Complied status").ToString()
                            rttbl.Rows(i)("% Complied") = dt.Rows(k).Item("% Complied").ToString()
                        End If
                    Next
                End If
            Next
