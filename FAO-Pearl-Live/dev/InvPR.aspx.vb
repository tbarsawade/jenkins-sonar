    ...
    Public Shared Function GetData(sdate As String, tdate As String) As DGrid
        ...
        Try
            ...
            If HttpContext.Current.Session("USERROLE") = "AP-TEAM" Then
                da.SelectCommand.CommandText = "select distinct d.tid from mmm_mst_doc d inner join mmm_doc_dtl dt on dt.docid=d.tid and dt.aprstatus='Validation' where eid=" & HttpContext.Current.Session("EID") & " and documenttype='Invoice PO' and convert(date,dt.tdate)>='" & sdate & "' and convert(date,dt.tdate)<='" & tdate & "' and curstatus in ('Update of Payment Details') and fld33='" & HttpContext.Current.Session("UID") & "'"
            Else
                da.SelectCommand.CommandText = "select distinct d.tid from mmm_mst_doc d inner join mmm_doc_dtl dt on dt.docid=d.tid and dt.aprstatus='Validation' where eid=" & HttpContext.Current.Session("EID") & " and documenttype='Invoice PO' and convert(date,dt.tdate)>='" & sdate & "' and convert(date,dt.tdate)<='" & tdate & "' and curstatus in ('Update of Payment Details')"
                '' earlier code - bkup on 14-may-18 by sp
                ' da.SelectCommand.CommandText = "select distinct tid from mmm_mst_doc where eid=" & HttpContext.Current.Session("EID") & " and documenttype='Invoice PO' and convert(date,adate)>='" & sdate & "' and convert(date,adate)<='" & tdate & "' and curstatus in ('Update of Payment Details')"
            End If

            'fld33
            ' FOR DOC ID FROM DOCUMENT TABLE
            da.Fill(ds, "docid")
            da.SelectCommand.CommandText = "select qry from mmm_print_template where template_name='Invoice PR' and eid=" & HttpContext.Current.Session("EID") & ""
            da.Fill(ds, "data")
            ...
            If ds.Tables("data").Rows.Count > 0 Then
                For i = 0 To ds.Tables("docid").Rows.Count - 1   '' main doc table loop 
                    ...
                    da.SelectCommand.CommandType = CommandType.Text
                    da.SelectCommand.CommandText = "select sql_child2 from mmm_print_template where template_name='Invoice PR' and eid=" & HttpContext.Current.Session("EID") & ""  ' qyr for Vendor row (one for each invoice header 
                    da.Fill(ds, "VENDOR")
                    ...
                    If ds.Tables("child").Rows.Count > 0 Then
                        For j = 0 To ds.Tables("child").Rows.Count - 1
                            ...
                            da.SelectCommand.CommandType = CommandType.Text
                            da.SelectCommand.CommandText = "select sql_childitem from mmm_print_template where template_name='Invoice PR' and eid=" & HttpContext.Current.Session("EID") & ""
                            ' FOR ALLOCATION
                            da.Fill(ds, "PR")
                            ...
                            If ds.Tables("PRDATA").Rows.Count > 0 Then
                                ...
                                For k = 0 To ds.Tables("PRDATA").Rows.Count - 1
                                    da.SelectCommand.CommandType = CommandType.Text
                                    da.SelectCommand.CommandText = "select sql_child1 from mmm_print_template where template_name='Invoice PR' and eid=" & HttpContext.Current.Session("EID") & ""
                                    da.Fill(ds, "DOCDATA")
