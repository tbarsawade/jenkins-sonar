...
Partial Class FWChkPTMReport
    ...
    Public Sub BindCreatedBy()
        ...
        Try
            ...
            ' da.SelectCommand.CommandText = "select userrole,emailid,UID , case when (eid=180) then  UserName +'--' +UserID   else UserName end  UserName from mmm_mst_user with(nolock)  where eid=180 and userrole <> 'vendor' and userrole <> 'TEMP VENDOR' and isauth=1  order by UserName"
            da.SelectCommand.CommandText = "select UserName,ab.UID  from (select username + '--' + userid [UserName],UID From mmm_mst_user where eid=180 and userrole='requester'  union select u.username + '--' + u.userid  [UserName],u.UID From mmm_ref_role_user ref inner join mmm_mst_user U on u.uid=ref.uid and u.eid=ref.eid where ref.eid=180 and ref.rolename='requester' ) ab order by username"
    ...
    End Sub
    <WebMethod()>
    Public Shared Function GetData(sdate As String, edate As String, PRNO As String, CreatedBy As String, Department As String, Status As String, Entity As String, Vendor As String) As DGrid
        ...
        Try
            ...
            da.SelectCommand.CommandText = "FourWayChkReport_PTM"
            ...
            da.SelectCommand.Parameters.AddWithValue("@PRNO", Trim(PRNO))
            ...
            '    da.SelectCommand.CommandText = "select qryfield[cmd] from mmm_mst_report with(nolock) where reportid=2122"
            'Else
            '    da.SelectCommand.CommandText = "select qryfieldrole[cmd] from mmm_mst_report with(nolock) where reportid=2122"
            'End If
            'da.Fill(ds, "qry")
            'qry = ds.Tables("qry").Rows(0).Item(0).ToString()
            'qry = Replace(qry, "@Frdate", sdate)
            'qry = Replace(qry, "@Todate", edate)
            'qry = Replace(qry, "@PRNO", PRNO)
            'qry = Replace(qry, "@CreatedBy", CreatedBy)
            'qry = Replace(qry, "@Department", Department)
            'qry = Replace(qry, "@Status", Status)
            'qry = Replace(qry, "@Entity", Entity)
            'qry = Replace(qry, "@Vendor", Vendor)
