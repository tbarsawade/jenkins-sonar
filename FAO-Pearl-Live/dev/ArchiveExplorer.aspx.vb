...
Partial Class ArchiveExplorer
        ...
        If Not Session("ACTION") Is Nothing Then
            ...
            If dt.Rows.Count > 0 Then
                If Session("ACTION").ToString() = "APPROVAL" Then
                    ...
                    ob.CreateControlsOnPanel(dt, pnlFields, updatePanelApprove, btnApprove, 1)
                Else
                    lblHeaderPopUp.Text = dt.Rows(0).Item("Formcaption").ToString()
                    Dim ob As New DynamicForm()
    ...
    Private Sub LoadWorkGroupTree()
        ...
        Dim STR As String = "select distinct year(adate) AS [Year],MONTH(aDate)[ordMonth],DateName( month , DateAdd( month , MONTH(aDate) , -1 ) )[Month] ,count(tid)[cnt] , documenttype ,curstatus from MMM_MST_DOC where EID=" & Session("EID") & " and curstatus='archive' group by year(adate),MONTH(aDate),cnt ,documenttype,curstatus  order by ordMonth,documenttype"
        Dim od As New SqlDataAdapter("", con)
        Dim ds As New DataSet
        od.SelectCommand.CommandText = STR
        od.Fill(ds, "data")
        Dim masterNode As New TreeNode("Year Wise Documnets")
        ...
        For Each rw As DataRow In ds.Tables("data").Rows
            If year <> rw.Item("year") Then
                year = rw.Item("Year")
                Dim n As New TreeNode()
                n.Text = rw.Item("Year").ToString()
                n.Value = rw.Item("Year")
                n.ImageUrl = "images/newuser.png"
                masterNode.ChildNodes.Add(n)
                LoadDocTree(ds.Tables("data"), n, year)
            End If
        Next
        lblFolderName.Text = "Selected STATUS : None"
        updTV.Update()
    End Sub
    Private Sub LoadDocTree(ByVal dt As DataTable, ByRef node As TreeNode, ByVal Year As Integer)
        Dim month As String = ""
        Dim dtrow As DataRow() = dt.Select("year=" & Year & "")
        For Each rw As DataRow In dtrow
            Dim n As New TreeNode()
            If month <> rw("Month").ToString() Then
                month = rw("Month").ToString()
                n.Text = rw("Month").ToString()
                n.Value = rw("ordMonth").ToString()
                n.ImageUrl = "images/folder.png"
                node.ChildNodes.Add(n)
                loadDocumentTypeTree(dtrow, n, month)
            End If
        Next
    End Sub
    Private Sub loadDocumentTypeTree(ByVal dt As DataRow(), ByVal node As TreeNode, ByVal month As String)

        Dim doctype As String = ""
        For Each rw As DataRow In dt
            Dim n As New TreeNode()
            If doctype <> rw("documenttype").ToString() And month = rw.Item("month").ToString() Then
                doctype = rw("documenttype").ToString()
                n.Text = rw("documenttype").ToString() & " ( " & rw("cnt").ToString() & " )"
    ...
    Private Sub BindDataGrid()
        If Not tv.SelectedNode Is Nothing Then
            ...
            Dim od As New SqlDataAdapter("select D.tid,D.EID,D.DocumentType,D.aDate,U.UserName from MMM_MST_DOC D left outer join MMM_MST_USER U on D.oUID=U.uid  where D.EID=" & Session("EID") & " and D.curstatus='archive'  and MONTH(aDate)=" & arr(2) & " And Year(aDate)=" & gid & " and documenttype='" & arr(3) & "' ", con)
    ...
    Protected Sub ShowApprove(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If ds.Tables("fields").Rows.Count > 0 Then
            ...
            ob.CreateControlsOnPanel(ds.Tables("fields"), pnlFields, updatePanelApprove, btnApprove, 1)
        End If
        con.Close()
        oda.Dispose()
        con.Dispose()
    ...
    Protected Sub BindGridbyDocidorFname(ByVal docid As String, ByVal Fname As String)
        Dim conStr As String = ConfigurationManager.ConnectionStrings("conStr").ConnectionString
        Dim con As SqlConnection = New SqlConnection(conStr)
        Dim od As New SqlDataAdapter("select D.tid,D.EID,D.DocumentType,D.aDate,U.UserName from MMM_MST_DOC D left outer join MMM_MST_USER U on D.oUID=U.uid  where D.EID=" & Session("EID") & " and D.curstatus='archive'  and tid=" & docid & "", con)
        Dim ds As New DataSet
        od.Fill(ds, "data")
        If ds.Tables("data").Rows.Count > 0 Then
            gvData.DataSource() = ds.Tables("data")
