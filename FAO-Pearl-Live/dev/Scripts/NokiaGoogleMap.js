...
function GetData(eid,branchtype)
{
    debugger;
       $.ajax({
        type: "post",
        url: "MasterMap.aspx/ConvertDataTabletoString",
...
{ 
    CreatePolygonbyNokiaMap(abc.d, branchtype);
}
...
function sdm(sd)
{
    ...
    function CreatePolygonbyNokiaMap(geopoints)
    {
        ...
        if (geopoints[GeoPointexist].hasOwnProperty('GeoPoint'))
            DrawGeoPoints(geopoints);
        if (geopoints[GeoPointexist].hasOwnProperty('GeoFence'))
            DrawGeoFence(geopoints);
    }
    ...
    function DrawGeoPoints(geopoints) {
        ...
        var marker = new google.maps.Marker([y1, y2], {
            position: myLatlng,
            map: map,
            title: data.title
        });
    }
    function DrawGeoFence(geopoints) {
        var arrLat = new Array();

        var arrLong = new Array();
        var GeoFencelength = geopoints.length;
        for (var i = 0; i < GeoFencelength; i++) {
            vpp = new Array();
            temp = 0;
