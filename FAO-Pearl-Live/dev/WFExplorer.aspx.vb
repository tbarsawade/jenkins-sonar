...
Partial Class WFExplorer
    ...
    Private Sub LoadWorkGroupTree()
        ...
        If Session("USERROLE").ToString() = "USR" Then
            STR &= " where isWorkFlow=1 and M.userid =" & Session("UID") & " and D.eid=" & Session("EID") & " group by UserName,M.userid,documenttype,aprstatus,curstatus order by username  , aprstatus "
        Else
            STR &= " where isWorkFlow=1 and M.userid<>217 and D.eid=" & Session("EID") & "  group by UserName,M.userid,documenttype,aprstatus,curstatus order by username  , aprstatus"
        End If
    ...
    Protected Sub ShowHit(ByVal sender As Object, ByVal e As EventArgs)
        ...
        sb.Append("window.open('" & strRoot & "/DOCS/" & Session("EID") & "/" & docURl & "');")
